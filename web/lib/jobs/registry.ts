...
console.log(`[RUN_FLYWHEEL_CYCLE] iteration=${result.iteration?.iterationNumber?? "-"} ` +
...
throw new Error(`[RUN_FLYWHEEL_CYCLE] iteration ${result.iteration.iterationNumber} failed: ${result.iteration.errorMessage?? "unknown error"}`)
}
} catch (err) {
...
// Purpose: 异步补全flywheel fields attributionConfidence,and 写 pflid.outcome_attribution
jobRuntime.register({
...
console.error(`[COMPUTE_OUTCOME_ATTRIBUTION] failed for ${recommendationId}:`,
...
// Purpose: 异步补全flywheel fields outcomeStability + horizonAgreement
jobRuntime.register({
...
console.error(`[COMPUTE_LONGITUDINAL_STABILITY] failed for ${recommendationId}:`,
(err as Error).message)
throw err
}
},
...
// Purpose: 异步补全flywheel fields outcomeClarity (based on on delayed_rewards 聚合 proxyReward)
jobRuntime.register({
...
console.error(`[COMPUTE_DELAYED_REWARD_PROXY] failed for ${recommendationId}:`,
(err as Error).message)
throw err
}
},
