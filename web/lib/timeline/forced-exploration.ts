...
/**
* records that an arm was exposed (selected) for a given request.
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data: rows, error } = await (supabase as any).from("arm_exposure_log").select("arm_id, exposure_count").eq("segment", segment).gte("bucket_start", windowStart.toISOString()).lte("bucket_start", windowEnd.toISOString())
...
const below = totalExposures < input.candidateArms.length? true: rate < config.minExposurePct
return {
armId,
exposureCount: count,
exposureRate: rate,
...
const below = stats.arms.filter((a) => a.belowMinThreshold).sort((a, b) => a.exposureCount - b.exposureCount)
