// =============================================
// Phase 3.7: Rollback Root reason Analyzer
// Timeline-First Architecture — Automated Root reason Diagnosis
// =============================================
...
export interface RootreasonCandidate {
cause: string
...
export interface RollbackRootreasonResult {
...
rootreasons: RootreasonCandidate[]
...
export interface RootreasonInput {
rollbackEventId?: string
timeRangeHours?: number
}

// ─── Root reason Analyzer ────────────────────────────────────────────────────
...
*/
export async function analyzeRollbackRootreason(input: RootreasonInput): Promise<RollbackRootreasonResult> {
...
const { data } = await (supabase as any).from("rollout_event_log").select("*").eq("id", input.rollbackEventId).single()
rollbackEvent = data
}
...
if (!rollbackEvent) {
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data } = await (supabase as any).from("rollout_event_log").select("*").in("event_type", ["rollback", "auto_rollback"]).order("created_at", { ascending: false }).limit(1).single()
rollbackEvent = data
}
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data: traces } = await (supabase as any).from("decision_trace_log").select("*").gte("created_at", cutoffTime).lte("created_at", rollbackTime).order("created_at", { ascending: true })
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data: events } = await (supabase as any).from("rollout_event_log").select("*").gte("created_at", cutoffTime).lte("created_at", rollbackTime).order("created_at", { ascending: true })
...
const rootreasons = identifyRootreasons(traces?? [], rollbackEvent, preRollbackState)
...
async function reconstructPreRollbackState(supabase: unknown,
rollbackTime: string): Promise<RollbackRootreasonResult["preRollbackState"]> {
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data: state } = await (supabase as any).from("rollout_state").select("*").single()

// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data: flags } = await (supabase as any).from("feature_flags").select("flag_key,flag_value")
...
function identifyRootreasons(traces: Record<string, unknown>[],
rollbackEvent: Record<string, unknown>,
preRollbackState: RollbackRootreasonResult["preRollbackState"]): RootreasonCandidate[] {
const causes: RootreasonCandidate[] = []

// reason 1: Latency spike analysis
const latencyreason = analyzeLatencySpike(traces)
if (latencyreason) causes.push(latencyreason)

// reason 2: Fallback rate analysis
const fallbackreason = analyzeFallbackRate(traces)
if (fallbackreason) causes.push(fallbackreason)

// reason 3: Traffic composition change
const trafficreason = analyzeTrafficChange(traces, preRollbackState)
if (trafficreason) causes.push(trafficreason)

// reason 4: Flag change correlation
const flagreason = analyzeFlagChange(rollbackEvent)
if (flagreason) causes.push(flagreason)
...
function analyzeLatencySpike(traces: Record<string, unknown>[]): RootreasonCandidate | null {
const latencies = traces.map((t) => t.latency_ms as number | null).filter((l): l is number => l!== null && l > 0)
...
function analyzeFallbackRate(traces: Record<string, unknown>[]): RootreasonCandidate | null {
if (traces.length < 10) return null

const fallbackCount = traces.filter((t) => t.decision_path === "FALLBACK" || t.decision_path === "MASTER_OFF" || t.decision_path === "ROLLBACK").length
...
function analyzeTrafficChange(traces: Record<string, unknown>[],
preRollbackState: RollbackRootreasonResult["preRollbackState"]): RootreasonCandidate | null {
const trafficPct = preRollbackState.timelineTrafficPct

// If traffic was high (>50%) and rollback happened, traffic may be a factor
if (trafficPct > 50) {
const timelineCount = traces.filter((t) => t.decision_path!== "FALLBACK" && t.decision_path!== "MASTER_OFF" && t.decision_path!== "ROLLBACK").length
...
function analyzeFlagChange(rollbackEvent: Record<string, unknown>): RootreasonCandidate | null {
