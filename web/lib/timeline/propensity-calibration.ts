...
export async function loadCalibrationConfig(): Promise<PropensityCalibrationConfig> {
 ...
 maxCalibrationRatio:
 typeof flag.max_calibration_ratio === "number"? flag.max_calibration_ratio: DEFAULT_CALIBRATION_CONFIG.maxCalibrationRatio,
 }
 } catch {
...
export async function computeObservedPropensities(input: {
 ...
 const { data: rows, error } = await (supabase as any).from("bandit_rewards").select("arm_id").eq("segment", segment).gte("created_at", windowStart.toISOString()).lte("created_at", windowEnd.toISOString())
...
export async function getCalibrationRatio(input: {
 ...
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 const { data, error } = await (supabase as any).from("propensity_calibration").select("calibration_ratio").eq("arm_id", input.armId).eq("segment", input.segment?? "global").gte("computed_at", cutoff).order("computed_at", { ascending: false }).limit(1).single()
