...
const healthScoreDelta = input.healthScore!== undefined && input.healthScoreBaseline!== undefined? input.healthScore - input.healthScoreBaseline: null
...
const { data, error } = await (supabase as any).schema("pflid").from("longitudinal_outcomes").insert({
pet_id: input.petId,
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
let query = (supabase as any).schema("pflid").from("longitudinal_outcomes").select("*").order("measured_at", { ascending: false })
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
let query = (supabase as any).schema("pflid").from("longitudinal_outcomes").select("outcome_class, health_score_delta")
