...
const ranked = [...states].filter((s) => s.total_pulls >= config.minSamples).sort((a, b) => b.mean_reward - a.mean_reward)
...
const targetTimeline = clamp(bestArm.mean_reward,
config.weightFloor,
...
const proposedTimeline = clamp(current.timeline + cappedDeltaTimeline,
config.weightFloor,
config.weightCeiling)
const proposedReview = clamp(1 - proposedTimeline,
config.weightFloor,
config.weightCeiling)
...
*/
export async function applyWeightAdjustments(propositions: WeightProposition[],
options: { triggeredBy?: "auto" | "manual"; reason?: string } = {}): Promise<ApplyResult> {
...
const { error: armErr } = await (supabase as any).from("bandit_arms").update({
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data: snapRow, error: snapErr } = await (supabase as any).from("adaptive_weight_snapshots").insert({
arm_id: prop.armId,
