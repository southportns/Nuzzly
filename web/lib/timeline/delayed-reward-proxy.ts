...
retentionWindowDays:
typeof flag.retention_window_days === "number"? flag.retention_window_days: DEFAULT_DELAYED_CONFIG.retentionWindowDays,
maxLookbackDays:
...
/**
* records a delayed reward event (called by retention-tracking batch job
...
const { data, error } = await (supabase as any).from("delayed_rewards").insert({
...
// eslint-disable-next-line @typescript-eslint/no-explicitany
const { data: rows, error } = await (supabase as any).from("delayed_rewards").select("event_type, event_value, created_at").eq("arm_id", input.armId).eq("segment", segment).gte("created_at", windowStart.toISOString()).lte("created_at", windowEnd.toISOString())
...
const { data: rows, error } = await (supabase as any).from("delayed_rewards").select("id, request_id, arm_id, event_value, segment").eq("applied_to_bandit", false).limit(limit)
...
if (result) {
// eslint-disable-next-line @typescript-eslint/no-explicitany
await (supabase as any).from("delayed_rewards").update({ applied_to_bandit: true, applied_at: new Date().toISOString() }).eq("id", row.id)
applied++
} else {
skipped++
}
