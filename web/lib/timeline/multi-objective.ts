 ...
 */
export function normalizeObjectives(raw: Record<ObjectiveKey, number>,
 bounds: Record<ObjectiveKey, { min: number; max: number }>): Record<ObjectiveKey, number> {
 ...
 */
export function computeCompositeScore(normalized: Record<ObjectiveKey, number>,
 weights: ObjectiveWeights): number {
 ...
 */
function dominates(a: Record<ObjectiveKey, number>,
 b: Record<ObjectiveKey, number>): boolean {
 ...
 */
export function computeParetoFrontier(candidates: Array<{ armId: string; scores: Record<ObjectiveKey, number> }>): ParetoPoint[] {
 ...
 */
export function adjustWeightsForContext(base: ObjectiveWeights,
 context: {
...
export async function saveObjectiveWeights(weights: ObjectiveWeights, policyVersion: string): Promise<void> {
 ...
 await (supabase as any).from("global_policy_config").update({ objective_weights: weights }).eq("version", policyVersion).eq("status", "draft")
}
