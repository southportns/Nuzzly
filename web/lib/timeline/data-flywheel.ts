...
const { data: iterationData, error: iterError } = await (supabase as any).schema("pflid").from("flywheel_iterations").insert({
...
outcomeHistory: outcomeSuccess? [{ outcome: "success", confidence: successProb }]: [{ outcome: "failure", confidence: 1 - successProb }],
})
...
const dataCompleteness = input.recommendations.length > 0? attributions / input.recommendations.length: 0

// 7. Mark iteration as completed
// eslint-disable-next-line @typescript-eslint/no-explicit-any
await (supabase as any).schema("pflid").from("flywheel_iterations").update({
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data, error } = await (supabase as any).schema("pflid").from("flywheel_iterations").select("iteration_number").order("iteration_number", { ascending: false }).limit(1)
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data, error } = await (supabase as any).schema("pflid").from("flywheel_iterations").select("*").eq("id", id).single()
