...
export async function recommendFoodByOutcome(petId: string,
 limit = 5): Promise<OutcomeRecommendationResult | null> {
...
export async function matchOutcomeForPet(input: OutcomeRecommendationInput,
 limit = 10): Promise<OutcomeRecommendationOutput[]> {
 const supabase = await createClient()
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export async function getOutcomeIntel(productId: string): Promise<OutcomeIntel | null> {
 const supabase = await createClient()
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export async function batchGetOutcomeIntel(productIds: string[]): Promise<Map<string, OutcomeIntel>> {
