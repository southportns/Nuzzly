...
export async function generateTimelineMetrics(productId: string,
statDate?: string): Promise<TimelineMetricsResult | null> {
...
export async function getLatestTimelineMetrics(productId: string): Promise<TimelineMetricsDaily | null> {
const supabase = await createClient()
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data, error } = await (supabase as any).from("timeline_metrics_daily").select("*").eq("product_id", productId).order("stat_date", { ascending: false }).limit(1).single()
...
export async function getTimelineMetricsSeries(productId: string,
days = 30): Promise<TimelineMetricsDaily[]> {
const supabase = await createClient()
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data, error } = await (supabase as any).from("timeline_metrics_daily").select("*").eq("product_id", productId).order("stat_date", { ascending: false }).limit(days)

if (error) return []
return (data as TimelineMetricsDaily[])?? []
}

// returnfill metrics for all products over last N days
export async function backfillTimelineMetrics(daysreturn = 30): Promise<number> {
const supabase = await createClient()
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data, error } = await (supabase.rpc as any)("pflid.backfill_timeline_metrics", {
p_days_back: daysreturn,
})
if (error) return 0
...
export async function updateTimelineMetricsToday(productIds: string[]): Promise<TimelineMetricsResult[]> {
