...
// 自动from 生产Data库表构造 FlywheelInput,trigger data flywheel cycle
// =============================================
// Data来源映射:
...
// 旧Datacancan still has'accepted',兼容两
...
/**
* from 生产Data库构造 FlywheelInput.
* Defaultused createClient() (server.ts);for RLS Limit for owner-only 表
* (recommendation_trace_log / recommendation_feedback) 回退to admin client.
*/
...
// 1. recommendation_trace_log: Last 30 daysRecommendedrecords (RLS: trace_read_own → admin)
const { data: traceLogsData } = await admin.from("recommendation_trace_log").select("id, pet_id, model_version, user_segment, feature_snapshot, input_features, created_at").gte("created_at", thirtyDaysAgoIso).order("created_at", { ascending: false })
...
if (!productId) continue // no product_id No flywheel
...
// 2. recommendation_feedback: this recommendation_id Feedback (RLS: feedback_read_own → admin)
const recIds = recommendations.map(r => r.recommendationId)
const feedbackByRec = new Map<string, FeedbackRow[]>()
if (recIds.length > 0) {
const { data: feedbackData } = await admin.from("recommendation_feedback").select("recommendation_id, action, pet_id, product_id").in("recommendation_id", recIds)
...
// 3. health_metrics: 每only Pet appetite/stool/activity Change (RLS: read_auth → createClient)
const petIds = Array.from(new Set(recommendations.map(r => r.petId)))
const healthByPet = new Map<string, HealthMetricRow[]>()
if (petIds.length > 0) {
const { data: hmData } = await supabase.from("health_metrics").select("pet_id, date, appetite_score, stool_score, activity_score").in("pet_id", petIds).gte("date", thirtyDaysAgoDate).order("date", { ascending: true })
...
// 4. pet_events: 每only Pet Events数 + Timenew 鲜度 (RLS: events_read_auth → createClient)
const eventsByPet = new Map<string, PetEventRow[]>()
if (petIds.length > 0) {
const { data: evData } = await supabase.from("pet_events").select("pet_id, event_time, event_type").in("pet_id", petIds).gte("event_time", thirtyDaysAgoIso).order("event_time", { ascending: false })
...
// 5. pets: use on cohort 构造 (RLS: pets_select_auth → createClient)
const petsById = new Map<string, PetRow>()
if (petIds.length > 0) {
const { data: petsData } = await supabase.from("pets").select("id, species, life_stage, stomach_health, breed").in("id", petIds)
...
// 优先from feature_snapshot 读取true实flywheel fields,缺失时Downgradeto Default值
...
// Statistics:feature_snapshot Whether包含至few 1 true实flywheel fields
if ([
banditConfidenceRaw,
segmentAlignmentRaw,
outcomeClarityRaw,
...
// 7. benchmarkUpdates: from health_metrics 聚合 (按Symptom类some分组)
...
// 10. Logs:Statistics feature_snapshot true实flywheel fields覆盖率
const totalRecs = recommendations.length
const realPct =
totalRecs > 0? Math.round((realFlywheelFieldRecCount / totalRecs) * 100): 0
console.log(`[flywheel-input-builder] ${totalRecs} RecommendedRecord, ${realFlywheelFieldRecCount} trueflywheel fields(Percentage ${realPct}%)`)
...
function extractString(featureSnap: Record<string, unknown>,
...
const parts = [m.appetite_score, m.stool_score, m.activity_score].filter((v): v is number => v!= null)
...
// events already按 event_time desc Sort,取Latesta
...
// 按 symptom category 分组:stool_score → soft_stool_improvement etc.
...
function buildCohortData(petsById: Map<string, PetRow>,
healthByPet: Map<string, HealthMetricRow[]>,
recommendations: FlywheelInput["recommendations"],
outcomes: FlywheelInput["outcomes"]): FlywheelInput["cohortData"] {
// 按 species / life_stage / stomach_health 三 Dimension构造 cohort
...
ensureCohort(`stomach_health:${pet.stomach_health}`,
{ stomach_health: pet.stomach_health },
pet.id)
}
}

// 收集每 recommendation product score (use healthScoreDelta 作 for 代理)
...
// use 该 pet recommendations healthScoreDelta 作 for effectiveness 代理
...
function computeBaselineMetrics(healthByPet: Map<string, HealthMetricRow[]>,
outcomes: FlywheelInput["outcomes"]): FlywheelInput["baselineMetrics"] {
...
avgHealthScoreList.length > 0? avgHealthScoreList.reduce((s, v) => s + v, 0) / avgHealthScoreList.length: 0.5

const improvementRate =
totalWithDelta > 0? improvedCount / totalWithDelta: 0.3

// effectivenessScore: use outcomes 正to delta Percentage作 for 代理 (0-100)
const outcomeValues = Object.values(outcomes)
const effectivenessScore =
outcomeValues.length > 0? (outcomeValues.filter(o => o.healthScoreDelta > 0).length / outcomeValues.length) * 100: 50
