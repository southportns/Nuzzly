...
export async function buildTimelineContext(reviewText: string,
 productId: string,
 petId?: string): Promise<TimelineContext | null> {
...
export async function buildTimelineContextFromDB(productId: string,
 petId?: string): Promise<TimelineContext | null> {
 const supabase = await createClient()

 // Get timeline groups for this product
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 const { data: groups } = await (supabase as any).schema("pflid").from("review_timeline_groups").select("id, timeline_score, review_count").eq("product_id", productId).eq("is_active", true).limit(10)
 ...
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 const { data: events } = await (supabase as any).schema("pflid").from("review_timeline_events").select("*").in("timeline_group_id", groupIds).order("event_day", { ascending: true }).limit(200)
 ...
 const trustScore = groups.reduce((sum: number, g: { timeline_score: number | null; review_count: number }) =>
 sum + (g.timeline_score?? 50) * g.review_count,
 0) / groups.reduce((sum: number, g: { review_count: number }) => sum + g.review_count,
 0)
 return {
