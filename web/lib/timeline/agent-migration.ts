...
async function enrichWithTimelineScores(reviewResult: PipelineResult,
_petId: string,
decision: RolloutDecision,
requestId: string,
t0: number,
userId?: string): Promise<ShadowRecommendationResult> {
...
const scoreDelta = topRec? topRec.timeline_score - topRec.review_score: null
...
function buildTimelineExplanation(usedTimeline: boolean,
timelineScore: number,
reviewScore: number,
comparison: Awaited<ReturnType<typeof calculateScoreComparison>>): string {
if (!usedTimeline) {
return ` based on onCommentRating (${reviewScore.toFixed(0)}),No Timeline Data`
}

const delta = timelineScore - reviewScore
const deltaStr = delta > 0? `high onCommentRating ${delta.toFixed(0)} `: `low onCommentRating ${Math.abs(delta).toFixed(0)} `

let explanation = `to Rating ${timelineScore.toFixed(0)} (${deltaStr})`

if (comparison && Math.abs(comparison.score_delta) > 15) {
if (comparison.score_delta < 0) {
explanation += " | Note:Timeline Ratinglow onCommentRating,cancan "
} else {
explanation += " |:Timeline long onCommentRating"
}
}
...
export async function scoreProductTimelineOnly(productId: string,
petId: string): Promise<{
score: number
timeline_score: number
stability_rate: number
soft_stool_risk: number
