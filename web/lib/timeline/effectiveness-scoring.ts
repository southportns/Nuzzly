...
export async function saveEffectivenessScore(input: EffectivenessInput): Promise<EffectivenessScore | null> {
 ...
 const { data: existing } = await (supabase as any).schema("pflid").from("effectiveness_scores").select("version").eq("entity_type", input.entityType).eq("entity_id", input.entityId).order("version", { ascending: false }).limit(1)

 const nextVersion = (existing?.[0]?.version?? 0) + 1

 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 const { data, error } = await (supabase as any).schema("pflid").from("effectiveness_scores").insert({
...
export async function getEffectivenessScores(params: {
 ...
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 let query = (supabase as any).schema("pflid").from("effectiveness_scores").select("*").order("effectiveness_score", { ascending: false })
...
export async function getTopEffectiveEntities(entityType: EntityType,
 limit = 10): Promise<EffectivenessScore[]> {
