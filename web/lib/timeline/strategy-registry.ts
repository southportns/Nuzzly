...
let q = (supabase as any).from("strategy_registry").select("*").order("created_at", { ascending: false }).limit(filter?.limit?? 100)
...
const supabase = await createClient()
// eslint-disable-next-line @typescript-eslint/no-explicitany
const { data, error } = await (supabase as any).from("strategy_registry").select("*").eq("strategy_id", strategyId).single()
...
const supabase = await createClient()
// eslint-disable-next-line @typescript-eslint/no-explicitany
const { data, error } = await (supabase as any).from("strategy_registry").insert({
...
// eslint-disable-next-line @typescript-eslint/no-explicitany
await (supabase as any).from("strategy_registry").update({ status: "retired", retired_at: new Date().toISOString() }).eq("arm_id", input.arm_id).eq("status", "active").neq("strategy_id", data.strategy_id)
}
...
export async function setStrategyStatus(strategyId: string,
status: StrategyStatus): Promise<StrategyRecord | null> {
const supabase = await createClient()
const patch: Record<string, unknown> = { status }
if (status === "retired") patch.retired_at = new Date().toISOString()

// eslint-disable-next-line @typescript-eslint/no-explicitany
const { data, error } = await (supabase as any).from("strategy_registry").update(patch).eq("strategy_id", strategyId).select("*").single()
...
options: { limit?: number; sinceHours?: number } = {}): Promise<StrategyPerformanceRow[]> {
const supabase = await createClient()
const limit = options.limit?? 100
// eslint-disable-next-line @typescript-eslint/no-explicitany
let q = (supabase as any).from("strategy_performance_history").select("*").eq("strategy_id", strategyId).order("recorded_at", { ascending: false }).limit(limit)
...
*/
export async function getStrategySummary(strategyId: string,
options: { sinceHours?: number } = {}): Promise<{
...
totalPulls > 0? history.reduce((acc, h) => acc + h.mean_reward * h.pulls, 0) / totalPulls: 0
...
function weightedMean<T>(rows: T[],
field: keyof T,
weightField: keyof T,
extractValue: (row: T, key: keyof T) => number | null,
extractWeight: (row: T, key: keyof T) => number): number {
let sumVal = 0
let sumWeight = 0
for (const r of rows) {
