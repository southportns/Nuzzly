...
export async function saveAttribution(input: AttributionInput & {
 ...
 const { data, error } = await (supabase as any).schema("pflid").from("outcome_attribution").insert({
...
export async function getAttributionForRecommendation(recommendationId: string): Promise<AttributionResult[]> {
 const supabase = await createClient()

 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 const { data, error } = await (supabase as any).schema("pflid").from("outcome_attribution").select("*").eq("recommendation_id", recommendationId).order("outcome_window_days", { ascending: true })
...
export async function getAttributionStats(params: {
 ...
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 let query = (supabase as any).schema("pflid").from("outcome_attribution").select("outcome_success, outcome_confidence, contribution_timeline, contribution_strategy, contribution_bandit, contribution_segment, contribution_random")
