...
const { data, error } = await (supabase as any).from("bandit_arms").select("*").eq("is_active", true).order("arm_id")
...
const supabase = await createClient()
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data, error } = await (supabase as any).from("bandit_state").select("*").eq("segment", segment).in("arm_id", armIds)
...
return {
armId: fallback,
weightConfig: decision.timelineWeight > 0? { timeline: decision.timelineWeight, review: decision.reviewWeight }: { timeline: 0, review: 1 },
...
// records exposure for the selected arm (best-effort).
...
// Fetch current state
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data: current } = await (supabase as any).from("bandit_state").select("*").eq("arm_id", input.armId).eq("segment", segment).single()
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data: updated, error } = await (supabase as any).from("bandit_state").update({
alpha: newAlpha,
beta: newBeta,
...
}).eq("arm_id", input.armId).eq("segment", segment).select("*").single()
