...
function findSimilarCases(outcomeHistory: Array<{ outcome: string; confidence: number }>): SimilarCase[] {
return outcomeHistory.filter((o) => o.outcome === "success").slice(0, 3).map((o, i) => ({
...
const signalDesc = improvingSignals.length > 0? `${improvingSignals.length} positive health signal(s) detected`: "no strong positive signals detected"

const evidenceDesc = evidenceCount > 0? `supported by ${evidenceCount} piece(s) of evidence`: "limited supporting evidence"

const similarDesc = similarCaseCount > 0? `${similarCaseCount} similar successful case(s) found`: "no similar cases found"
...
const { data, error } = await (supabase as any).schema("pflid").from("explainability_records").insert({
...
export async function getExplainabilityForRecommendation(recommendationId: string): Promise<ExplainabilityRecord | null> {
const supabase = await createClient()

// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data, error } = await (supabase as any).schema("pflid").from("explainability_records").select("*").eq("recommendation_id", recommendationId).order("created_at", { ascending: false }).limit(1).single()
