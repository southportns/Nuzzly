...
const { data: rewardRows, error: rErr } = await (supabase as any).from("bandit_rewards").select("arm_id, reward, request_id, created_at").eq("segment", segment).gte("created_at", windowStart.toISOString()).lte("created_at", windowEnd.toISOString())
...
// eslint-disable-next-line @typescript-eslint/no-explicitany
const { count } = await (supabase as any).from("replay_snapshots").select("id", { count: "exact", head: true }).in("request_id", requestIds)
...
statistical_method: useCalibration? useReplay? "ips_calibrated_with_replay": "ips_calibrated": useReplay? "ips_empirical_with_replay": "ips_empirical",
})
}
...
const bestLiftCi: [number, number] = best? [best.ipsCiLower - baselineMean, best.ipsCiUpper - baselineMean]: [0, 0]
...
bestLiftCi,
statisticalMethod: useCalibration? useReplay? "ips_calibrated_with_replay": "ips_calibrated": useReplay? "ips_empirical_with_replay": "ips_empirical",
...
// eslint-disable-next-line @typescript-eslint/no-explicitany
const { data: rows } = await (supabase as any).from("bandit_rewards").select("arm_id, reward").eq("segment", segment).in("arm_id", [input.armA, input.armB]).gte("created_at", windowStart.toISOString()).lte("created_at", windowEnd.toISOString())
