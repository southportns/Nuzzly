...
export function computeCohortMetrics(params: {
 ...
 const avgHealthScore = healthScores.length > 0? healthScores.reduce((s, v) => s + v, 0) / healthScores.length: 0

 const improvementRate = improvementRates.length > 0? improvementRates.reduce((s, v) => s + v, 0) / improvementRates.length: 0

 const effectivenessScore = effectivenessScores.length > 0? effectivenessScores.reduce((s, v) => s + v, 0) / effectivenessScores.length: 0

 // Top products by score
 const topProducts = Object.entries(productScores).sort(([, a], [, b]) => b - a).slice(0, 5).map(([productId, score]) => ({ productId, score }))
 ...
 */
export function compareWithBaseline(cohort: CohortMetrics,
 baseline: {
 avgHealthScore: number
 improvementRate: number
 effectivenessScore: number
 ...
 health_score_ratio: baseline.avgHealthScore > 0? round2(cohort.avgHealthScore / baseline.avgHealthScore): null,
 improvement_rate_ratio: baseline.improvementRate > 0? round2(cohort.improvementRate / baseline.improvementRate): null,
 }
}
...
export async function saveCohortIntelligence(input: {
 ...
 const { data: existing } = await (supabase as any).schema("pflid").from("cohort_intelligence").select("version").eq("cohort_key", input.cohortKey).order("version", { ascending: false }).limit(1)

 const nextVersion = (existing?.[0]?.version?? 0) + 1

 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 const { data, error } = await (supabase as any).schema("pflid").from("cohort_intelligence").insert({
...
export async function getCohortIntelligence(params?: {
 ...
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 let query = (supabase as any).schema("pflid").from("cohort_intelligence").select("*").order("cohort_key")
...
export async function compareCohorts(cohortKeys: string[]): Promise<{
 ...
 const filtered = cohortKeys.length > 0? cohorts.filter((c) => cohortKeys.includes(c.cohortKey)): cohorts
