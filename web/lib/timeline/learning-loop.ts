...
export async function runNearlineLoop(input: Partial<NearlineLoopConfig> = {}): Promise<NearlineAggregation> {
 ...
 const { data: rows, error } = await (supabase as any).from("bandit_rewards").select("arm_id, reward, reward_components, created_at").eq("segment", config.segment).gte("created_at", windowStart.toISOString()).lte("created_at", windowEnd.toISOString())
...
export async function runOfflineLoop(input: OfflineLoopInput = {}): Promise<OfflineLoopResult> {
 ...
 // eslint-disable-next-line @typescript-eslint/no-explicitany
 const { data: rewardRows, error: rErr } = await (supabase as any).from("bandit_rewards").select("arm_id, reward, created_at").eq("segment", segment).gte("created_at", windowStart.toISOString()).lte("created_at", windowEnd.toISOString())
...
function groupArmMetrics(rows: Array<{ arm_id: string; reward: number; reward_components: Record<string, number> | null; created_at: string }>): Map<string, ArmMetricAccum> {
...
async function persistStrategyPerformance(input: {
 ...
 const supabase = await createClient()
 // eslint-disable-next-line @typescript-eslint/no-explicitany
 const { data: strat } = await (supabase as any).from("strategy_registry").select("strategy_id").eq("arm_id", input.armId).eq("status", "active").order("created_at", { ascending: false }).limit(1).single()
