...
const { error } = await (supabase as any).from("decision_trace_log").insert(entries.map((e) => ({
...
const supabase = await createClient()
// eslint-disable-next-line @typescript-eslint/no-explicit-any
let query = (supabase as any).from("decision_trace_log").select("*").order("created_at", { ascending: false }).limit(filters.limit?? 100)
