...
*/
export function mutateStrategy(parent: StrategyRecord,
mutationStrength = 0.1): SynthesisCandidate {
...
*/
export function crossoverStrategies(parent1: StrategyRecord,
parent2: StrategyRecord,
alpha = 0.5): SynthesisCandidate {
const allKeys = new Set([...Object.keys(parent1.weight_config),...Object.keys(parent2.weight_config),
])
...
let q = (supabase as any).from("strategy_performance_history").select("strategy_id, arm_id, mean_reward, pulls").gte("recorded_at", cutoff).gte("pulls", minPulls).order("mean_reward", { ascending: false }).limit(1)
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data } = await (supabase as any).from("strategy_performance_history").select("strategy_id, arm_id, mean_reward, pulls, recorded_at").gte("recorded_at", cutoff).gte("pulls", 10).order("recorded_at", { ascending: false })
