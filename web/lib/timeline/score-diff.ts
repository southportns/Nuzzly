...
export async function computeDiff(input: DiffInput): Promise<DiffResult> {
 ...
 const { data } = await (supabase as any).from("decision_trace_log").select("*").eq("request_id", input.requestId).single()
 trace = data
 }
 ...
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 const { data: snapshot } = await (supabase as any).from("replay_snapshots").select("*").eq("request_id", input.requestId).single()

 if (!snapshot) {
 throw new Error(`No scoring data found for request_id: ${input.requestId}`)
 }

 return computeDiffFromData(input.requestId?? "unknown",
...
function computeDiffFromData(requestId: string,
 ...
 */
function computeKendallTau(listA: Array<[string, number]>,
