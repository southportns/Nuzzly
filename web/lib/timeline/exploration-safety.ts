...
maxExplorationPct:
typeof flag.max_exploration_pct === "number"? flag.max_exploration_pct: DEFAULT_SAFETY_THRESHOLDS.maxExplorationPct,
}
} catch {
...
const { data: rows, error } = await (supabase as any).from("bandit_rewards").select("arm_id, reward, reward_components, created_at").eq("segment", segment).gte("created_at", cutoff)
...
// CTR drop check
const ctrDropPct = baseline && baseline.ctr > 0? ((baseline.ctr - stats.ctr) / baseline.ctr) * 100: 0
...
const convDropPct = baseline && baseline.conversionRate > 0? ((baseline.conversionRate - stats.conversionRate) / baseline.conversionRate) * 100: 0
...
function computeArmStats(_armId: string,
bucket?: { rewards: number[]; components: Array<Record<string, number>> }): { sampleSize: number; ctr: number; conversionRate: number; meanReward: number } | null {
...
try {
await executeRollback(`Bandit safety: ${input.trigger} on arm=${input.armId}: ${input.reason}`,
"auto")
} catch (err) {
...
await pauseExploration(`Guardrail ${check.trigger} fired on ${check.armId} (drop=${check.dropPct.toFixed(2)}%)`)
}
}
