...
export async function activatePolicy(version: string, reason?: string): Promise<GlobalPolicyConfig | null> {
 ...
 await (supabase as any).from("global_policy_config").update({ status: "archived", archived_at: new Date().toISOString() }).eq("status", "active")

 // 2. Activate the new policy
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 const { data, error } = await (supabase as any).from("global_policy_config").update({
 status: "active",
 activated_at: new Date().toISOString(),
 metadata: { activated_reason: reason?? "Manual activation" },
 }).eq("version", version).eq("status", "draft").select("*").single()
...
export async function getActivePolicy(): Promise<GlobalPolicyConfig | null> {
 const supabase = await createClient()
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 const { data } = await (supabase as any).from("global_policy_config").select("*").eq("status", "active").order("activated_at", { ascending: false }).limit(1).single()
...
export async function getPolicyHistory(limit = 20): Promise<unknown[]> {
 const supabase = await createClient()
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 const { data } = await (supabase as any).from("policy_history").select("*").order("created_at", { ascending: false }).limit(limit)
...
export async function listPolicyConfigs(options: {
 ...
 const supabase = await createClient()
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 let q = (supabase as any).from("global_policy_config").select("*").order("created_at", { ascending: false }).limit(options.limit?? 50)
