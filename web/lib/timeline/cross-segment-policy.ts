...
export async function listSegmentPolicies(): Promise<SegmentPolicy[]> {
 ...
 const { data, error } = await (supabase as any).from("segment_policies").select("*").eq("is_active", true).order("segment_key")
...
export async function getSegmentPolicy(segmentKey: SegmentKey): Promise<SegmentPolicy | null> {
 const supabase = await createClient()
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 const { data } = await (supabase as any).from("segment_policies").select("*").eq("segment_key", segmentKey).single()
 ...
 */
export async function computeSegmentAdjustment(segmentKey: SegmentKey,
 globalWeights: ObjectiveWeights): Promise<SegmentPolicyAdjustment> {
 ...
 */
export async function computeAllSegmentAdjustments(globalWeights: ObjectiveWeights): Promise<Record<SegmentKey, SegmentPolicyAdjustment>> {
...
export async function updateSegmentPolicy(input: {
 ...
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 const { data, error } = await (supabase as any).from("segment_policies").update(patch).eq("segment_key", input.segmentKey).select("*").single()
