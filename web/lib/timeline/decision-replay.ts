...
throw new Error(`Cannot satisfy target fidelity ${input.targetFidelity}: trace only supports ${fidelity.level}. ` +
`Warnings: ${fidelity.warnings.join("; ")}`)
}

// Step 3: Create replay job record
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data: job } = await (supabase as any).from("replay_jobs").insert({
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
await (supabase as any).from("replay_jobs").update({
status: "completed",
replayed_trace: {
...
const supabase = await createClient()
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data } = await (supabase as any).from("replay_jobs").select("*").eq("id", jobId).single()
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data } = await (supabase as any).from("decision_trace_log").select("*").eq("request_id", requestId).single()
...
async function replayRolloutDecision(originalTrace: Record<string, unknown>,
mode?: string): Promise<ReplayStep> {
const t0 = performance.now()
...
function analyzeDivergence(originalTrace: Record<string, unknown>,
replayedDecision: ReplayResult["replayedDecision"]): ReplayResult["divergence"] {
...
const divergenceReason = hasDivergence? `Divergence detected in: ${divergentSteps.join(", ")}. Original: ${originalPath}, Replayed: ${replayedPath}`: null
...
async function fetchCachedReplay(// eslint-disable-next-line @typescript-eslint/no-explicit-any
supabase: any,
idempotencyKey: string): Promise<Record<string, unknown> | null> {
const { data } = await supabase.from("replay_jobs").select("*").eq("idempotency_key", idempotencyKey).eq("status", "completed").order("completed_at", { ascending: false }).limit(1).single()
return data?? null
}

function buildResultFromCached(cached: Record<string, unknown>,
input: ReplayInput,
idempotentHit: boolean): ReplayResult {
...
function assessFidelity(trace: Record<string, unknown>,
_target?: FidelityLevel): FidelityAssessment {
...
featureFlagsFull: ["TIMELINE_ENABLED", "AB_TEST_ENABLED", "TIMELINE_ROLLOUT_PCT"].every((k) => flags[k]!== undefined),
}
...
if (available.featureFlagsSnapshot &&
