...
const { data, error } = await (supabase as any).from("feature_flags").select("flag_value").eq("flag_key", key).eq("environment", environment).single()
...
export function evaluatePercentageRollout(userId: string | undefined,
