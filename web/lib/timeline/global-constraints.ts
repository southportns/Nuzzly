...
export async function loadHardConstraints(): Promise<HardConstraints> {
 ...
 const { data } = await (supabase as any).from("global_policy_config").select("hard_constraints").eq("status", "active").order("activated_at", { ascending: false }).limit(1).single()
...
export async function loadSoftConstraints(): Promise<SoftConstraints> {
 const supabase = await createClient()
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 const { data } = await (supabase as any).from("global_policy_config").select("soft_constraints").eq("status", "active").order("activated_at", { ascending: false }).limit(1).single()
 ...
 */
export function checkRollbackSafety(isRollbackActive: boolean,
 hard: HardConstraints): ConstraintViolation | null {
 ...
 */
export function checkMinQuality(qualityScore: number,
 hard: HardConstraints,
 segment?: SegmentKey,
 armId?: string): ConstraintViolation | null {
 ...
 */
export function checkDiversity(diversityIndex: number,
 soft: SoftConstraints): ConstraintViolation | null {
 ...
 */
export function checkFairness(segmentExposures: Record<string, number>,
 soft: SoftConstraints): ConstraintViolation[] {
 ...
 */
export function checkExplorationCap(explorationPct: number,
 soft: SoftConstraints,
 segment?: SegmentKey): ConstraintViolation | null {
...
export async function getUnresolvedViolations(limit = 50): Promise<ConstraintViolation[]> {
 const supabase = await createClient()
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 const { data } = await (supabase as any).from("constraint_violation_log").select("*").eq("resolved", false).order("created_at", { ascending: false }).limit(limit)
