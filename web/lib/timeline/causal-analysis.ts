...
export async function performCausalAnalysis(input: CausalAnalysisInput): Promise<CausalAnalysisResult> {
 ...
 const { data: traces } = await (supabase as any).from("decision_trace_log").select("*").gte("created_at", input.timeRangeStart).lte("created_at", input.timeRangeEnd).order("created_at", { ascending: true })
 ...
 const confounderAnalysis = controlDimensions.length > 0? analyzeConfounders(traces, groupBy, metric, controlDimensions): []

 // Persist confounder results
 if (confounderAnalysis.length > 0) {
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 await (supabase as any).from("causal_confounders").insert(confounderAnalysis.map((c) => ({
 ...
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 await (supabase as any).from("causal_analysis_results").insert({
 analysis_id: analysisId,
 ...
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 await (supabase as any).from("bootstrap_results").insert({
 analysis_id: analysisId,
 metric_name: metric,
...
function groupTraces(traces: Record<string, unknown>[],
 groupBy: string): Record<string, Record<string, unknown>[]> {
...
function computeGroupMetrics(groups: Record<string, Record<string, unknown>[]>,
 metric: string,
 controlDimensions: ConfounderType[]): Record<string, TraceMetrics> {
...
function compareGroups(metrics: Record<string, TraceMetrics>): CausalAnalysisResult["groupComparison"] {
 ...
 */
function performBootstrapTest(metrics: Record<string, TraceMetrics>,
 iterations: number,
 confidenceLevel: number): BootstrapResult {
 const keys = Object.keys(metrics)
 if (keys.length < 2) {
 return {
 observedValue: 0,
 ...
 return {
 observedValue: valuesA.length > 0 && valuesB.length > 0? valuesA.reduce((a, b) => a + b, 0) / valuesA.length - valuesB.reduce((a, b) => a + b, 0) / valuesB.length: 0,
 bootstrapMean: 0,
 bootstrapStd: 0,
 ciLower: 0,
 ciUpper: 0,
...
function computeCohensD(valuesA: number[], valuesB: number[]): number {
 ...
 const pooled = Math.sqrt(((valuesA.length - 1) * sA ** 2 + (valuesB.length - 1) * sB ** 2) /
...
function analyzeConfounders(traces: Record<string, unknown>[],
 groupBy: string,
 metric: string,
 controlDimensions: ConfounderType[]): ConfounderResult[] {
...
async function correlateWithFlagChanges(supabase: unknown,
 timeStart: string,
 timeEnd: string,
 _groupMetrics: Record<string, TraceMetrics>): Promise<CausalAnalysisResult["flagChangeCorrelations"]> {
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 const { data: events } = await (supabase as any).from("rollout_event_log").select("*").gte("created_at", timeStart).lte("created_at", timeEnd).order("created_at", { ascending: true })
...
function generateHypotheses(comparison: CausalAnalysisResult["groupComparison"],
 bootstrap: BootstrapResult,
 _flagCorrelations: CausalAnalysisResult["flagChangeCorrelations"],
 groupBy: string,
 confounderAnalysis: ConfounderResult[]): CausalHypothesis[] {
 ...
 evidence.push(`Top confounder: ${topConfounder.confounderType}=${topConfounder.bucket} (delta ${topConfounder.delta})`)
 }
...
function detectSimpsonsParadox(comparison: CausalAnalysisResult["groupComparison"],
 confounderAnalysis: ConfounderResult[]): boolean {
...
function buildTimeSeries(traces: Record<string, unknown>[],
 groupBy: string): CausalAnalysisResult["timeSeriesData"] {
 ...
 const isGroupA = groupBy === "ab_bucket"? trace.ab_group === "control": trace.decision_path === "FALLBACK" || trace.decision_path === "MASTER_OFF"
 ...
 return Object.entries(hourlyBuckets).sort(([a], [b]) => a.localeCompare(b)).map(([timestamp, data]) => ({
 timestamp,
 groupA_avg: data.groupA.length > 0? round(mean(data.groupA)): 0,
 groupB_avg: data.groupB.length > 0? round(mean(data.groupB)): 0,
 rollout_pct: data.rolloutPct.length > 0? Math.round(mean(data.rolloutPct)): 0,
 }))
}
