...
export async function executeRollback(reason: string,
 type: RollbackType = "full"): Promise<RollbackResult> {
...
export async function checkAndAutoRollback(): Promise<RollbackResult | null> {
 ...
 const supabase = await createClient()
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 const { data: thresholdsData } = await (supabase as any).from("feature_flags").select("flag_value").eq("flag_key", FLAG_KEYS.AUTO_ROLLBACK_THRESHOLDS).single()
 ...
 return executeRollback(`Auto-rollback: failure rate ${failureRate.toFixed(1)}% > ${thresholds.failureRatePct}%`,
 "auto")
 }

 // Check latency
 if (metrics.scoring_latency_ms.p95 > thresholds.latencyP95Ms) {
 return executeRollback(`Auto-rollback: P95 latency ${metrics.scoring_latency_ms.p95}ms > ${thresholds.latencyP95Ms}ms`,
 "auto")
 }
 ...
 return executeRollback(`Auto-rollback: score drift ${avgDelta.toFixed(1)} > ${thresholds.scoreDriftThreshold}`,
 "auto")
 }
...
export async function getRollbackHistory(limit = 20): Promise<unknown[]> {
 const supabase = await createClient()
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 const { data } = await (supabase as any).from("rollout_event_log").select("*").in("event_type", ["rollback", "auto_rollback"]).order("created_at", { ascending: false }).limit(limit)
