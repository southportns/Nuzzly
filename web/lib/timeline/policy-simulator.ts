...
const segmentResults = computeSegmentResults(replayData,
baselineResults,
simulatedResults,
policy)
...
function computeScoresForData(data: ReplayRow[],
weights: ObjectiveWeights): ScoredRow[] {
return data.map((row) => {
...
function computeSegmentResults(data: ReplayRow[],
baseline: ScoredRow[],
simulated: ScoredRow[],
policy: GlobalPolicyConfig): Record<string, SegmentSimulationResult> {
...
const avgBaseline =
segmentBaseline.length > 0? segmentBaseline.reduce((s, r) => s + r.compositeScore, 0) / segmentBaseline.length: 0

const avgSimulated =
segmentSimulated.length > 0? segmentSimulated.reduce((s, r) => s + r.compositeScore, 0) / segmentSimulated.length: 0
...
function computeSystemUplift(segmentResults: Record<string, SegmentSimulationResult>): SystemUplift {
...
const weightedUplift = segments.reduce((s, r) => s + r.uplift * r.sampleSize,
0) / totalSamples

const weightedUpliftPct = segments.reduce((s, r) => s + r.upliftPct * r.sampleSize,
0) / totalSamples
...
function determineRecommendation(uplift: SystemUplift,
constraintsPassed: boolean): "approve" | "reject" | "neutral" | "needs_tuning" {
...
async function loadReplayData(windowHours: number,
segments?: SegmentKey[]): Promise<ReplayRow[]> {
const supabase = await createClient()
const cutoff = new Date(Date.now() - windowHours * 3600_000).toISOString()

// eslint-disable-next-line @typescript-eslint/no-explicit-any
let q = (supabase as any).from("bandit_rewards").select("arm_id, segment, reward, reward_components, created_at").gte("created_at", cutoff).order("created_at", { ascending: false }).limit(10000)
...
async function resolvePolicy(input: SimulationInput): Promise<GlobalPolicyConfig | null> {
...
const supabase = await createClient()
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data } = await (supabase as any).from("global_policy_config").select("*").eq("version", input.policyVersion).single()
...
const supabase = await createClient()
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data } = await (supabase as any).from("global_policy_config").select("objective_weights").eq("status", "active").order("activated_at", { ascending: false }).limit(1).single()
...
function makefailedResult(simulationId: string,
input: SimulationInput,
errorMessage: string): SimulationResult {
...
const supabase = await createClient()
// eslint-disable-next-line @typescript-eslint/no-explicit-any
let q = (supabase as any).from("policy_simulation_results").select("*").order("created_at", { ascending: false }).limit(options.limit?? 20)
