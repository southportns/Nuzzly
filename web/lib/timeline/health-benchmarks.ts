...
export function computeBenchmarkStats(input: BenchmarkUpdate): {
 ...
 const marginOfError = n >= 30? 1.96 * stdDev / Math.sqrt(n): 2.045 * stdDev / Math.sqrt(n) // t-value for small samples

 // Days to improvement percentiles
 const daysSorted = [...input.daysToImprovement].sort((a, b) => a - b)
 const medianDays = daysSorted.length > 0? daysSorted[Math.floor(daysSorted.length / 2)]: null
 const p75Days = daysSorted.length > 0? daysSorted[Math.floor(daysSorted.length * 0.75)]: null
...
export async function updateBenchmark(input: BenchmarkUpdate): Promise<HealthBenchmark | null> {
 ...
 await (supabase as any).schema("pflid").from("health_benchmarks").update({ valid_to: new Date().toISOString() }).eq("category", input.category).eq("subcategory", input.subcategory?? "").is("valid_to", null)

 // Get next version
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 const { data: existing } = await (supabase as any).schema("pflid").from("health_benchmarks").select("version").eq("category", input.category).eq("subcategory", input.subcategory?? "").order("version", { ascending: false }).limit(1)

 const nextVersion = (existing?.[0]?.version?? 0) + 1

 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 const { data, error } = await (supabase as any).schema("pflid").from("health_benchmarks").insert({
...
export async function getBenchmarks(params?: {
 ...
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
 let query = (supabase as any).schema("pflid").from("health_benchmarks").select("*").is("valid_to", null) // Only current versions.order("category")
