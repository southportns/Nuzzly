...
const events = checkpoint? await this.loadEventsAfter(def, checkpoint.createdAt): await this.loadAllEvents(def)

// 3. If checkpoint exists, start from checkpoint state
let state = checkpoint? (checkpoint.stateSnapshot?? def.initialState): def.initialState
...
*/
async replayByAggregate(projectionName: string,
aggregateId: string): Promise<ReplayResult> {
const def = projectionRegistry.findByName(projectionName)
if (!def) {
throw new Error(`[ProjectionRebuilder] Unknown projection: ${projectionName}`)
}
...
*/
async rebuildFromVersion(projectionName: string,
fromVersion: number): Promise<RebuildResult> {
const def = projectionRegistry.findByName(projectionName)
if (!def) {
throw new Error(`[ProjectionRebuilder] Unknown projection: ${projectionName}`)
}
