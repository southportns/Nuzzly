...
const { data, error } = await admin.from("pet_disease_records").insert(record).select().single()
...
const admin = createAdminClient()
const { data, error } = await admin.from("pet_disease_records").update(patch).eq("id", id).select().single()
...
const admin = createAdminClient()
const { error } = await admin.from("pet_disease_records").delete().eq("id", id)
...
const admin = createAdminClient()
const { data, error } = await admin.from("pet_medication_records").insert(record).select().single()
...
const admin = createAdminClient()
const { data, error } = await admin.from("pet_medication_records").update(patch).eq("id", id).select().single()
...
const admin = createAdminClient()
const { error } = await admin.from("pet_medication_records").delete().eq("id", id)
...
const admin = createAdminClient()
const { data, error } = await admin.from("pet_attachments").insert(record).select().single()
...
const admin = createAdminClient()
const { error } = await admin.from("pet_attachments").delete().eq("id", id)
...
logged_date: record.logged_date ?? new Date().toISOString().split("T")[0],
}),
source: "api",
})
...
const { error } = await (admin as any).from("diet_logs").insert({...record, profile_id: userId })
...
const admin = createAdminClient()
const { error } = await admin.from("diet_logs").delete().eq("id", id)
...
const admin = createAdminClient()
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data, error } = await (admin as any).from("health_records").insert(record).select("id").single()
...
const admin = createAdminClient()
const { error } = await admin.from("health_records").delete().eq("id", id)
...
timestamp: Date.now(),
idempotencyKey: generateIdempotencyKey("UPDATE_PET_WEIGHT", { id: petId, weight_kg: String(weightKg) }),
source: "api",
})
} catch {
// Gateway failure should not block the actual DB write
...
const admin = createAdminClient()
const { error } = await admin.from("pets").update({ weight_kg: weightKg }).eq("id", petId)
...
const admin = createAdminClient()
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { error } = await (admin as any).from("pet_allergies").insert(record)
...
const admin = createAdminClient()
const { error } = await admin.from("pet_allergies").delete().eq("id", id)
...
const admin = createAdminClient()
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { error } = await (admin as any).from("environment_profiles").upsert(record, { onConflict: "pet_id" })
...
await getWriteGateway().submit({
id: crypto.randomUUID(),
type: "DELETE_ENVIRONMENT_PROFILE",
actor: userId,
payload: { pet_id: petId },
timestamp: Date.now(),
idempotencyKey: generateIdempotencyKey("DELETE_ENVIRONMENT_PROFILE", { pet_id: petId }),
source: "api",
})
} catch {
// Gateway failure should not block the actual DB write
...
const admin = createAdminClient()
const { error } = await admin.from("environment_profiles").delete().eq("pet_id", petId)
...
await getWriteGateway().submit({
id: crypto.randomUUID(),
type: "CREATE_HEALTH_REMINDER",
actor: userId,
payload: record as Record<string, unknown>,
timestamp: Date.now(),
idempotencyKey: generateIdempotencyKey("CREATE_HEALTH_REMINDER", {
pet_id: record.pet_id,
title: record.title,
...
const admin = createAdminClient()
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { error } = await (admin as any).from("health_reminders").insert(record)
...
await getWriteGateway().submit({
id: crypto.randomUUID(),
type: "UPDATE_HEALTH_REMINDER",
actor: userId,
payload: { id,...patch } as Record<string, unknown>,
timestamp: Date.now(),
idempotencyKey: generateIdempotencyKey("UPDATE_HEALTH_REMINDER", { id }),
source: "api",
})
} catch {
// Gateway failure should not block the actual DB write
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { error } = await (admin as any).from("health_reminders").update(update).eq("id", id).eq("profile_id", userId)
...
await getWriteGateway().submit({
id: crypto.randomUUID(),
type: "DELETE_HEALTH_REMINDER",
actor: userId,
payload: { id },
timestamp: Date.now(),
idempotencyKey: generateIdempotencyKey("DELETE_HEALTH_REMINDER", { id }),
source: "api",
})
} catch {
// Gateway failure should not block the actual DB write
...
const admin = createAdminClient()
const { error } = await admin.from("health_reminders").delete().eq("id", id).eq("profile_id", userId)
...
const { error } = await admin.from("pets").delete().eq("id", petId).eq("profile_id", userId) // Ensure user owns this pet
