...
export async function queryPetEvents(petId: string,
filter?: {
...
let query = supabase.from("pet_events").select("*").eq("pet_id", petId).order("event_time", { ascending: false })
...
const supabase = await createServerClient()
const { data, error } = await supabase.from("pet_events").select("*, pets!inner(name, breed), products(name, brand)").eq("profile_id", profileId).order("event_time", { ascending: false }).limit(limit)
return { data, error }
}
...
const supabase = await createServerClient()
let query = supabase.from("symptom_ontology").select("*").eq("is_active", true).order("category").order("display_name")
...
const supabase = await createServerClient()
const { data, error } = await supabase.from("symptom_ontology").select("*").or(`canonical_name.ilike.%${sanitized}%,display_name.ilike.%${sanitized}%`).eq("is_active", true)
return { data: data as SymptomOntology[] | null, error }
}

export async function findSymptomByAlias(alias: string) {
const supabase = await createServerClient()
const { data, error } = await supabase.from("symptom_ontology").select("*").contains("aliases", [alias]).eq("is_active", true).maybeSingle()
return { data: data as SymptomOntology | null, error }
}
...
const supabase = await createServerClient()
const { data, error } = await supabase.from("environment_profiles").select("*").eq("pet_id", petId).maybeSingle()
...
const supabase = await createServerClient()
const { data, error } = await supabase.from("environment_profiles").upsert({
pet_id: petId,
...
const supabase = await createServerClient()
let query = supabase.from("food_usage_periods").select("*, products(name, brand, image_url)").eq("pet_id", petId).order("start_date", { ascending: false })
...
export async function createFoodUsagePeriod(petId: string,
profileId: string,
productId: string,
startDate: string,
options?: {
...
await supabase.from("food_usage_periods").update({ is_current: false, end_date: new Date().toISOString().split("T")[0] }).eq("pet_id", petId).eq("is_current", true)

const { data, error } = await supabase.from("food_usage_periods").insert({
pet_id: petId,
...
export async function endFoodUsagePeriod(periodId: string,
options?: {
...
const supabase = await createServerClient()
const { error } = await supabase.from("food_usage_periods").update({
is_current: false,
...
const supabase = await createServerClient()
let query = supabase.from("health_records").select("*").eq("pet_id", petId).order("record_time", { ascending: false })
...
export async function createHealthRecord(petId: string,
profileId: string,
...
const supabase = await createServerClient()
const { data, error } = await supabase.from("life_stage_history").select("*").eq("pet_id", petId).order("start_date", { ascending: false })
...
const supabase = await createServerClient()
const { data, error } = await supabase.from("causal_event_chains").select("*").eq("pet_id", petId).order("created_at", { ascending: false })
...
const supabase = await createServerClient()
let query = supabase.from("stable_samples").select("*").eq("product_id", productId).order("stable_days", { ascending: false })
...
const supabase = await createServerClient()
const { data, error } = await supabase.from("stable_samples").select("stable_days, stability_score, sample_quality").eq("product_id", productId).order("stable_days", { ascending: false })
...
const supabase = await createServerClient()
const { data, error } = await supabase.from("data_trust_scores").select("*").eq("entity_type", entityType).eq("entity_id", entityId).maybeSingle()
...
const supabase = await createServerClient()
let query = supabase.from("nlp_extractions").select("*").order("created_at", { ascending: false }).limit(50)
...
const supabase = await createServerClient()
const { data, error } = await supabase.from("pet_complete_timeline").select("*").eq("pet_id", petId).order("event_time", { ascending: false }).limit(100)
return { data, error }
}

export async function queryBreedRiskAnalysis(breed?: string, species?:'cat' |'dog' |'other') {
const supabase = await createServerClient()
let query = supabase.from("breed_risk_analysis").select("*").order("occurrence_count", { ascending: false })
...
const supabase = await createServerClient()
const { data, error } = await supabase.from("food_long_term_outcomes").select("*").eq("product_id", productId).order("start_date", { ascending: false })
return { data, error }
}
