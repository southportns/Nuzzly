...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data: group, error: groupErr } = await (supabase as any).schema("pflid").from("review_timeline_groups").select("*").eq("id", timelineGroupId).single()

if (groupErr ||!group) return null

// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data: events } = await (supabase as any).schema("pflid").from("review_timeline_events").select("*").eq("timeline_group_id", timelineGroupId).order("event_day", { ascending: true })

// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data: mapping } = await (supabase as any).schema("pflid").from("review_to_timeline").select("source_review_id, review_date, review_order").eq("timeline_group_id", timelineGroupId).order("review_order", { ascending: true })

const reviewIds: string[] = (mapping as Array<{ source_review_id: string }>?? []).map((m) => m.source_review_id)
...
const { data: reviewData } = await supabase.from("product_reviews").select("id, review_text, overall_rating").in("id", reviewIds)

if (reviewData) {
const reviewMap = new Map(reviewData.map((r: { id: string; review_text: string | null; overall_rating: number | null }) => [r.id, r]))
reviewsWithText = reviewsWithText.map((r) => ({...r,
...
return {...group as unknown as TimelineGroup,
...
export async function queryProductTimelines(productId: string, limit = 20, offset = 0): Promise<{ timelines: TimelineGroup[]; total: number }> {
const supabase = await createServerClient()
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data, count, error } = await (supabase as any).schema("pflid").from("review_timeline_groups").select("*", { count: "exact" }).eq("product_id", productId).eq("is_active", true).order("total_days_span", { ascending: false }).range(offset, offset + limit - 1)
...
const supabase = await createServerClient()
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data, error } = await (supabase as any).schema("pflid").from("review_timeline_groups").select("*").eq("author_id", authorId).order("last_review_date", { ascending: false }).limit(limit)
...
const supabase = await createServerClient()
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data, error } = await (supabase as any).schema("pflid").from("review_timeline_events").select("*").eq("timeline_group_id", timelineGroupId).order("event_day", { ascending: true })
...
export async function upsertTimelineEvents(timelineGroupId: string, reviewId: string, sourceReviewId: string, events: Json[]): Promise<number> {
const supabase = await createServerClient()
// eslint-disable-next-line @typescript-eslint/no-explicit-any
