...
/**
* used Next.js unstable_cache 替代 存缓存
* Serverless 环境 更can靠,支持自动失效 and TagRefresh
*/
export const queryTotalPetCount = unstable_cache(async () => {
const supabase = createServiceRoleClient()
const { count, error } = await supabase.from("pets").select("*", { count: "exact", head: true }).eq("is_active", true)
...
const supabase = await createServerClient()
const { data, error } = await supabase.from("pets").select("*").eq("profile_id", profileId).eq("is_active", true).order("created_at", { ascending: false })
...
const supabase = await createServerClient()
const { data, error } = await supabase.from("notifications").select("*").eq("profile_id", profileId).order("created_at", { ascending: false }).limit(limit)
...
const supabase = await createServerClient()
const { data, error } = await supabase.from("diet_logs").select("*, products(name, brand)").eq("pet_id", petId).order("logged_date", { ascending: false }).limit(limit)
return { data, error }
}
...
const supabase = await createServerClient()
const { data, error } = await supabase.from("health_records").select("id, weight_kg, record_time").eq("pet_id", petId).eq("record_type", "weight").not("weight_kg", "is", null).order("record_time", { ascending: true }).limit(limit)

const processedData: WeightLogEntry[] | null = data?.map(d => ({
id: d.id,
weight_kg: d.weight_kg?? 0, // Security,empty
...
const supabase = await createServerClient()
const { data, error } = await supabase.from("review_followup_schedules").select("*, product_reviews(products(name, brand), pets(name))").eq("profile_id", profileId).eq("status", "pending").order("due_date", { ascending: true })
return { data, error }
}
