...
const { data, error } = await supabase.from('pet_medication_records').select('*').eq('pet_id', petId).order('started_on', { ascending: false })
...
const { data, error } = await supabase.from('pet_medication_records').select('*').eq('id', recordId).single()
...
const { data, error } = await supabase.from('pet_medication_records').insert(record).select().single()
...
const { data, error } = await supabase.from('pet_medication_records').update({...updates, updated_at: new Date().toISOString() }).eq('id', recordId).select().single()
...
const { error } = await supabase.from('pet_medication_records').delete().eq('id', recordId)
...
const { data, error } = await supabase.from('pet_medication_records').select('*').eq('pet_id', petId).eq('is_ongoing', true).order('started_on', { ascending: false })
