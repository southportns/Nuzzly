...
CREATE TABLE public.product_reviews (
  ...
  black_chin_rating     integer CHECK (black_chin_rating >= 1 AND black_chin_rating <= 5),
  vomit_rating          integer CHECK (vomit_rating >= 1 AND vomit_rating <= 5),
  tear_stain_rating     integer CHECK (tear_stain_rating >= 1 AND tear_stain_rating <= 5),
  shedding_rating       integer CHECK (shedding_rating >= 1 AND shedding_rating <= 5),
...
CREATE TABLE public.review_followups (
  ...
  black_chin_changes  text CHECK (black_chin_changes IN ('improved', 'worsened', 'unchanged', 'not_applicable')),
  vomit_changes       text CHECK (vomit_changes IN ('improved', 'worsened', 'unchanged', 'not_applicable')),
  tear_stain_changes  text CHECK (tear_stain_changes IN ('improved', 'worsened', 'unchanged', 'not_applicable')),
  shedding_changes    text CHECK (shedding_changes IN ('improved', 'worsened', 'unchanged', 'not_applicable')),
...
CREATE TABLE public.product_metrics_daily (
  ...
  black_chin_rate   numeric(5,2),
  vomit_rate        numeric(5,2),
  tear_stain_rate   numeric(5,2),
  shedding_rate     numeric(5,2),
...
CREATE INDEX idx_pet_weight_logs_pet_date ON public.pet_weight_logs(pet_id, logged_date);
...
CREATE POLICY "weight_manage_own" ON public.pet_weight_logs
  FOR ALL TO authenticated USING (
    pet_id IN (SELECT id FROM public.pets WHERE profile_id = auth.uid())
  ) WITH CHECK (
    pet_id IN (SELECT id FROM public.pets WHERE profile_id = auth.uid())
  );
...
BEGIN
  INSERT INTO public.product_metrics_daily (
    ...
    black_chin_rate, vomit_rate, tear_stain_rate, shedding_rate,
  ...
  SELECT
    ...
    -- 黑下巴发生率
    CASE
      WHEN COUNT(r.id) FILTER (WHERE r.black_chin_rating IS NOT NULL AND r.created_at::date = target_date) > 0
      THEN COUNT(*) FILTER (WHERE r.black_chin_rating <= 2 AND r.created_at::date = target_date)::float
           / COUNT(*) FILTER (WHERE r.black_chin_rating IS NOT NULL AND r.created_at::date = target_date)
      ELSE 0
    END,
    -- 呕吐发生率
    CASE
      WHEN COUNT(r.id) FILTER (WHERE r.vomit_rating IS NOT NULL AND r.created_at::date = target_date) > 0
      THEN COUNT(*) FILTER (WHERE r.vomit_rating <= 2 AND r.created_at::date = target_date)::float
           / COUNT(*) FILTER (WHERE r.vomit_rating IS NOT NULL AND r.created_at::date = target_date)
      ELSE 0
    END,
    -- 泪痕发生率
    CASE
      WHEN COUNT(r.id) FILTER (WHERE r.tear_stain_rating IS NOT NULL AND r.created_at::date = target_date) > 0
      THEN COUNT(*) FILTER (WHERE r.tear_stain_rating <= 2 AND r.created_at::date = target_date)::float
           / COUNT(*) FILTER (WHERE r.tear_stain_rating IS NOT NULL AND r.created_at::date = target_date)
      ELSE 0
    END,
    -- 掉毛发生率
    CASE
      WHEN COUNT(r.id) FILTER (WHERE r.shedding_rating IS NOT NULL AND r.created_at::date = target_date) > 0
      THEN COUNT(*) FILTER (WHERE r.shedding_rating <= 2 AND r.created_at::date = target_date)::float
           / COUNT(*) FILTER (WHERE r.shedding_rating IS NOT NULL AND r.created_at::date = target_date)
      ELSE 0
    END,
  ...
  ON CONFLICT (product_id, date) DO UPDATE SET
    ...
    vomit_rate = EXCLUDED.vomit_rate,
    tear_stain_rate = EXCLUDED.tear_stain_rate,
    shedding_rate = EXCLUDED.shedding_rate,
