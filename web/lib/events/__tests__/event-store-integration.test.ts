...
const { data: jobs } = await admin.from("pending_computation_jobs").select("*").eq("event_id", eventId)
...
const { data: jobs } = await admin.from("pending_computation_jobs").select("*").eq("job_type", "always_failing_job")
