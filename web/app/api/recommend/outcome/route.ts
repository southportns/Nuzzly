...
export async function POST(request: Request) {
 ...
 if (!result) {
 return NextResponse.json({ success: false, error: "Pet not found or no recommendations available" },
 { status: 404 })
 }
 ...
 if (context) {
 result.recommendations[0] = {...topProduct,
 timeline_context: {
 ...
 return NextResponse.json({ success: false, error: "Missing required fields: petId or (breed + age)" },
 { status: 400 })
 } catch (error) {
...
export async function GET(request: Request) {
 ...
 if (!productId) {
 return NextResponse.json({ success: false, error: "Missing product_id" },
 { status: 400 })
 }
