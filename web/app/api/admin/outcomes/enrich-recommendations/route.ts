...
// for 近 24h new 写 recommendation_trace_log 执行flywheel enrichment batch jobs:
...
// 支持 admin 鉴权 (Default) or cron 鉴权 (?trigger=cron + Authorization: Bearer ${CRON_SECRET})
...
// 鉴权: cron trigger校验 CRON_SECRET, No 走 admin 鉴权
if (isCron) {
...
// 1. 查询近 24h new 写 recommendation_trace_log
const { data: recentRecords, error: queryErr } = await admin.from("recommendation_trace_log").select("id").gte("created_at", since).order("created_at", { ascending: true })
...
// 2. 分 and 发: 每 CONCURRENCY, 每records 3 job and 行执行 (Promise.all)
// Limitand 发避免 DB 压力 (chunk 间串行, chunk 5 and 行)
