...
try {
const dataset = incremental? await incrementalOutcomeDataset(since): await buildFullOutcomeDataset(limit)

if (!dataset) {
return NextResponse.json({ success: false, error: "failed to build dataset" },
{ status: 500 })
}
