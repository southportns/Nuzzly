...
// 校验Pet归属
const { data: pet } = await db.from("pets").select("id, profile_id").eq("id", petId).maybeSingle()
if (!pet || pet.profile_id!== user.id) {
return NextResponse.json({ error: "NoPermissions" }, { status: 403 })
}

const { data, error } = await db.from("pet_disease_records").select("*").eq("pet_id", petId).order("diagnosed_on", { ascending: false })
...
const { pet_id, name, severity, status, diagnosed_on, recovered_on, symptoms, notes } = body

if (!pet_id ||!name) {
return NextResponse.json({ error: "pet_id and name Required" }, { status: 400 })
}
...
// 校验Pet归属
const { data: pet } = await db.from("pets").select("id, profile_id").eq("id", pet_id).maybeSingle()
if (!pet || pet.profile_id!== user.id) {
return NextResponse.json({ error: "NoPermissions" }, { status: 403 })
}

const { data, error } = await db.from("pet_disease_records").insert({
pet_id,
name: name.trim(),
severity: severity || "unknown",
status: status || "active",
diagnosed_on: diagnosed_on || null,
recovered_on: recovered_on || null,
