// GET/DELETE /api/ai/chat/history — ChatHistoryManagement
...
// 获取UserallPet ChatRecord
const { data, error } = await supabase.from("health_chat_sessions").select("id, user_message, ai_response, created_at, pet_id, model_used").eq("profile_id", user.id).order("created_at", { ascending: false }).limit(50)
...
const { error } = await supabase.from("health_chat_sessions").delete().eq("id", id).eq("profile_id", user.id)
