...
export async function GET(request: Request) {
 ...
 let query = db.from("health_reminders").select("*").eq("pet_id", petId).order("due_date", { ascending: true })
...
export async function POST(request: Request) {
 ...
 return NextResponse.json({ error: "petId, reminderType, title, dueDate Required" }, { status: 400 })
 }
 ...
 const { data, error } = await db.from("health_reminders").insert({
 pet_id: petId,
 profile_id: user.id,
