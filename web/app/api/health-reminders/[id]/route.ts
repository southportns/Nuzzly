...
// 校验all权
const { data: existing } = await db.from("health_reminders").select("id, profile_id").eq("id", id).maybeSingle()
if (!existing) return NextResponse.json({ error: "Reminderdoes not exist " }, { status: 404 })
...
return NextResponse.json({ error: "NoUpdate fields" }, { status: 400 })
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { data, error } = await (db as any).from("health_reminders").update(allowed).eq("id", id).select().single()
