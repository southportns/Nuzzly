...
// 获取Reminderand 校验all权
const { data: reminder, error: fetchErr } = await db.from("health_reminders").select("*").eq("id", id).maybeSingle()
if (!reminder) return NextResponse.json({ error: "Reminderdoes not exist " }, { status: 404 })
...
const { error: updateErr } = await db.from("health_reminders").update({ is_completed: true, completed_at: now }).eq("id", id)
...
// 检查Whether超past EndDate
if (!reminder.repeat_end_date || nextDueDate <= reminder.repeat_end_date) {
const { data: created, error: createErr } = await db.from("health_reminders").insert({
