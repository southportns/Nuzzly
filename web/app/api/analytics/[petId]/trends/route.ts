...
// 获取TrendsData
const { data: trends, error } = await supabase.rpc("get_health_trends", {
p_pet_id: petId,
p_days: days
}).single()
...
// 获取HealthMetricHistory
const { data: metrics } = await supabase.from("health_metrics").select("*").eq("pet_id", petId).gte("date", new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString().split("T")[0]).order("date", { ascending: true })
