// POST /api/analytics/trust-score — 计算某实体 Trust Score
// 接收: { entity_type, entity_id, metadata? }
// 鉴权: Bearer token 优先 (mobile), fallback cookie (web)
// 业务逻辑:
// - entity_type='user' → 读 profiles.trust_score
// - entity_type='product' → 读 product_reviews.overall_rating Average分
// - entity_type='pet' → 读 health_metrics close 联样本计算
// return: { trust_score, confidence, sample_size,... }
...
const { data, error } = await db.from("profiles").select("trust_score, review_count, long_term_review_count, verified_purchase_count, pet_profile_completeness").eq("id", entityId).maybeSingle()
...
// confidence: 0-1, based on on样本量 and long 期Data
...
// product: based on on product_reviews.overall_rating AverageRating映射to Trust Score
async function calcProductTrust(db: ReturnType<typeof createAdminClient>, entityId: string): Promise<TrustResult> {
const { data, error } = await db.from("product_reviews").select("overall_rating, has_voucher, verified_purchase, review_trust_score").eq("product_id", entityId)
...
const ratings = data.map((r) => r.overall_rating).filter((r): r is number => typeof r === "number" && r > 0)
const avgRating = ratings.length > 0? ratings.reduce((a, b) => a + b, 0) / ratings.length: 0
// 1-5 星Rating映射to 0-100 Trust Score
...
// pet: based on on health_metrics 样本量 and Stable性计算
async function calcPetTrust(db: ReturnType<typeof createAdminClient>, entityId: string): Promise<TrustResult> {
const { data, error } = await db.from("health_metrics").select("appetite_score, stool_score, activity_score, weight_delta, date").eq("pet_id", entityId).order("date", { ascending: false }).limit(30)
...
// 综合 trust_score: 三项Rating均值(false设 0-100)+ WeightStable性奖励
...
// 简单Abnormal检测:Ratingpast low or Fluctuatepast large 
...
return NextResponse.json({ error: `entity_type must be ${Array.from(ALLOWED_ENTITY_TYPES).join("/")}` },
{ status: 400 },)
}
...
// use admin 查(避免 RLS cookie↔Bearer 切换时 not a致)
...
break
default:
return NextResponse.json({ error: "does not support entity_type" }, { status: 400 })
}
