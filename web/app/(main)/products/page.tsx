...
return (<div className="bg-[#F7F7F5] min-h-screen">
...
<a
href="/products"
className={`rounded-full px-4 py-1.5 text-[13px] font-medium transition-colors ${!category? "bg-[#F59E0B] text-white": "bg-white text-[#6B7280] border border-[#E5E7EB]"
}`}
>
{tCommon("all")}
</a>
...
>
{isEn && (cat as Record<string, unknown>).name_en ? String((cat as Record<string, unknown>).name_en) : cat.name}
</a>))}
</div>)
}
...
let filtered = category? products?.filter((p) => p.product_categories?.slug === category): products
...
{filtered && filtered.length === 0 && (<div className="col-span-full py-20 text-center">
<div className="text-[48px]">🐱</div>
<p className="mt-4 text-[17px] font-medium text-[#1A1A1A]">{t("noMatchingProducts")}</p>
<p className="mt-1 text-[14px] text-[#6B7280]">{t("tryAdjustingFilters")}</p>
</div>)}
</div>)
}
...
{Array.from({ length: 4 }).map((_, i) => (<Skeleton key={i} className="h-9 w-full rounded-[12px]" />))}
</div>)
}

function ProductGridSkeleton() {
return (<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
{Array.from({ length: 8 }).map((_, i) => (<Skeleton key={i} className="h-56 rounded-[20px]" />))}
</div>)
}
