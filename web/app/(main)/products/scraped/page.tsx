...
title: "Scrapedproduct data — Nuzzly Town",
}

export default function ScrapedProductsPage() {
const products = getScrapedProducts()

return (<div className="bg-[#F7F6F3] min-h-screen">
...
from BoqiiScraped CatfoodData({products.length} products)
</p>
</div>
...
<p className="text-[17px] text-[#6B6B6B]">NoScrapedData</p>
</div>)}
</div>
</div>)
}
...
return (<div className="rounded-[24px] border border-[rgba(0,0,0,0.06)] bg-white overflow-hidden transition-all hover:shadow-[0_8px_40px_rgba(0,0,0,0.06)]">
{/* Product image */}
...
{/* Nutrition */}
{(product.proteinPercent || product.fatPercent) && (<div className="mt-3 flex gap-2 flex-wrap">
{product.proteinPercent && (<span className="text-[12px] bg-[#E8F5E9] text-[#2E7D32] px-2 py-1 rounded-full">
Protein {product.proteinPercent}%
</span>)}
{product.fatPercent && (<span className="text-[12px] bg-[#FFF3E0] text-[#E65100] px-2 py-1 rounded-full">
Fat {product.fatPercent}%
</span>)}
{product.fiberPercent && (<span className="text-[12px] bg-[#E3F2FD] text-[#1565C0] px-2 py-1 rounded-full">
Fiber {product.fiberPercent}%
</span>)}
</div>)}

{/* Ingredients preview */}
{product.ingredientList && product.ingredientList.length > 0 && (<div className="mt-3 pt-3 border-t border-[rgba(0,0,0,0.06)]">
<p className="text-[12px] text-[#6B6B6B] mb-1">Main Ingredients</p>
