...
return { success: false, reviewId: null, error: result.reason?? "Submitfailed" }
}
...
const { data: inserted, error: insertError } = await admin.from("product_reviews").insert({
product_id: record.product_id,
pet_id: record.pet_id,
profile_id: record.profile_id,
...
return { success: false, reviewId: null, error: insertError?.message?? "Insert failed" }
}

return { success: true, reviewId: inserted.id, error: null }
}

export async function submitReviewVoucherAction(reviewId: string,
file: File,
userId: string,) {
// 1. Upload to storage
const uploadResult = await uploadVoucher(file, reviewId)
if (!uploadResult.url) {
return { success: false, error: uploadResult.error?? "Uploadfailed" }
}
...
const admin = createAdminClient()
const { error } = await admin.from("review_vouchers").insert({
review_id: reviewId,
file_url: uploadResult.url,
file_type: fileType,
})
...
const admin = createAdminClient()
const { error } = await admin.from("product_bookmarks").insert({ profile_id: profileId, product_id: productId })
...
const admin = createAdminClient()
const { error } = await admin.from("product_bookmarks").delete().eq("profile_id", profileId).eq("product_id", productId)
