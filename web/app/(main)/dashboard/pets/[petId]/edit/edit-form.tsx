...
if (!user) return { ok: false as const, error: tPet("notSignIn") }
try {
// 1. Update pet
const { error: petErr } = await supabase.from("pets").update(payload.pet).eq("id", pet.id)
if (petErr) return { ok: false as const, error: petErr.message }

// 1.5. Handle avatar changes
if (avatarFile) {
const { data: petData } = await supabase.from("pets").select("photo_url").eq("id", pet.id).single()
...
if (delDisErr) console.warn("DeleteDiseaseRecordfailed:", delDisErr.message)
...
if (delMedErr) console.warn("DeleteMedicationRecordfailed:", delMedErr.message)
...
console.error("Save pet profile failed:", e)
