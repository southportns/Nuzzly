...
import { ProductComparison } from "@/components/ai/product-compare"
...
chat: { title: t("freeChat"), subtitle: t("freeChatDesc") },
recommend: { title: t("smartRecommendations"), subtitle: t("smartRecommendationsDesc") },
ingredients: { title: t("ingredientAnalysis"), subtitle: t("ingredientAnalysisDesc") },
compare: { title: t("productComparison"), subtitle: t("productComparisonDesc") },
}
...
{/* Sidebar */}
<div
className={cn("absolute z-50 left-0 top-0 bottom-0 w-64 shrink-0 transition-transform duration-300 md:relative md:z-auto md:translate-x-0",
...
{activeMode === "chat" && (<div className="flex items-center gap-1 rounded-full bg-[#A8C5A0]/15 px-2 py-0.5">
<span className="size-1.5 rounded-full bg-[#A8C5A0]" />
<span className="text-[10px] font-medium text-[#5A8A52]">{t("online")}</span>
</div>)}
</div>
...
{activeMode === "chat" && (<div className="flex items-center gap-2">
<button
...
{activeMode === "recommend" && (<div className="h-full overflow-y-auto px-4 py-6 md:px-8">
<div className="mx-auto max-w-2xl">
<RecommendationInput onRecommend={handleRecommend} loading={loading} />

{error && (<div className="mt-4 rounded-2xl border border-[#ff3b30]/20 bg-[#ff3b30]/5 p-4">
<p className="text-[14px] text-[#ff3b30]">
<EmojiIcon name="AlertTriangle" className="mr-2 inline size-4" />{error}
</p>
</div>)}

{result && (<div className="mt-6 space-y-4 pb-8">
{/* Summary */}
<div className="rounded-2xl border border-[#FF7A59]/20 bg-gradient-to-br from-[#FF7A59]/8 to-transparent p-4">
<p className="text-[14px]">
<span className="font-semibold text-[#FF7A59]">{t("analysisResult")}</span>
<span className="text-[#6B6B6B]"> — {result.summary}</span>
</p>
</div>

{/* Pet context */}
{result.pet_context && (<div className="flex flex-wrap items-center gap-2">
<span className="rounded-full bg-[#F0EFED] px-3 py-1 text-[13px] text-[#111111]">{t("breed")}: {result.pet_context.breed}</span>
<span className="rounded-full bg-[#F0EFED] px-3 py-1 text-[13px] text-[#111111]">{t("age")}: {result.pet_context.age}</span>
<span className={`rounded-full px-3 py-1 text-[13px] ${
result.pet_context.stomach_health === "sensitive" || result.pet_context.stomach_health === "very_sensitive"? "bg-[#ff3b30]/10 text-[#ff3b30]": "bg-[#F0EFED] text-[#111111]"
}`}>
{t("stomach")}: {result.pet_context.stomach_health === "sensitive" || result.pet_context.stomach_health === "very_sensitive" ? t("sensitive") : t("normal")}
</span>
</div>)}

{/* Recommendations */}
<div className="space-y-3">
{result.recommendations.filter((r) => r.product?.id).map((r, i) => (<RecommendationCard
key={r.product.id}
recommendation={r}
rank={i + 1}
...
{(result.warnings?.length?? 0) > 0 && (<div className="rounded-2xl border border-[#ff9500]/20 bg-[#ff9500]/5 p-4">
<p className="flex items-center gap-2 text-[14px] font-semibold text-[#ff9500]">
<EmojiIcon name="AlertTriangle" className="size-4" />{t("riskNotice")}
</p>
<div className="mt-2 space-y-1.5">
{result.warnings.filter((w) => w.product).map((w, i) => (<div key={i} className="flex items-center justify-between text-[14px]">
...
{result.trace_id && (<p className="mt-3 text-center text-[10px] text-[#6B6B6B]/50">
{t("decisionTraceId")}: {result.trace_id.slice(0, 8)}...
</p>)}
</div>)}
</div>
</div>)}

{activeMode === "ingredients" && (<div className="h-full overflow-y-auto px-4 py-6 md:px-8">
<div className="mx-auto max-w-2xl pb-8">
<IngredientAnalysis />
</div>
</div>)}

{activeMode === "compare" && (<div className="h-full overflow-y-auto px-4 py-6 md:px-8">
<div className="mx-auto max-w-2xl pb-8">
<ProductComparison />
</div>
</div>)}
