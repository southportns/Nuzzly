...
if (!dashJson.success) {
throw new Error(dashJson.error?? "Loading flywheelDatafailed")
}
...
throw new Error(json.error?? `Flywheel run failed (${res.status})`)
}
setRunStatus("completed")
toast.success("Flywheel iterationcompleted")
await fetchData()
await fetchIterations()
} catch (err) {
setRunStatus("failed")
const msg = err instanceof Error? err.message: "Flywheel run failed"
toast.error(msg)
} finally {
setRunning(false)
}
...
const stats = [
{ label: "Processed Recommendations", value: data.observability.flywheelCompletedCount, icon: emojiIcon("Sparkles"), accent: "#7BA7BC" },
{ label: "Outcome Analyses", value: data.observability.longitudinalCount, icon: emojiIcon("BarChart3"), accent: "#A8C5A0" },
{ label: "Attribution Calculations", value: data.observability.attributionCount, icon: emojiIcon("Target"), accent: "#E8A87C" },
{ label: "Benchmark Updates", value: data.observability.benchmarkUpdateCount, icon: emojiIcon("Layers"), accent: "#FF7A59" },
{ label: "effectiveness", value: data.observability.effectivenessScoreCount, icon: emojiIcon("Gauge"), accent: "#B59BD8" },
{ label: "Strategy Evaluations", value: data.observability.explainabilityCount, icon: emojiIcon("Activity"), accent: "#4A7A91" },
]
...
Core Asset Flywheel · Outcomes Dashboard
</h1>
<p className="mt-1 text-[14px] text-[#6B6B6B]">
Flywheel iterationMetric · Evidence Quality · effectiveness · Cohort Intelligence · HealthBenchmark
</p>
</div>
...
<section>
<h2 className="mb-3 text-[16px] font-semibold text-[#111111]">Flywheel iterationStatistics</h2>
...
<h2 className="text-[16px] font-semibold text-[#111111]">Flywheel iterationHistoryTrends</h2>
</div>
<Card>
<CardHeader>
<CardTitle className="text-sm">Evidence Quality & Data Completeness Trend</CardTitle>
<CardDescription>
Quality trends of last 30 iterations · blue line evidence_quality_score · green line data_completeness
</CardDescription>
</CardHeader>
<CardContent>
{iterationsLoading? (<Skeleton className="h-[200px] w-full" />): iterations.length === 0? (<div className="flex h-[200px] items-center justify-center text-sm text-muted-foreground">
No iteration data
</div>): (<IterationsTrendChart iterations={iterations} />)}
</CardContent>
</Card>
...
<CardTitle className="text-sm">Iteration Details</CardTitle>
</div>
<CardDescription>
Click row to expand and view errors. Status: running=yellow · completed=green · failed=red
</CardDescription>
</CardHeader>
<CardContent>
{iterationsLoading? (<Skeleton className="h-48 w-full" />): iterations.length === 0? (<div className="py-6 text-center text-sm text-muted-foreground">No iteration data</div>): (<IterationsTable iterations={iterations} />)}
</CardContent>
</Card>
</section>
...
<section>
<h2 className="mb-3 text-[16px] font-semibold text-[#111111]">Evidence Quality</h2>
<div className="grid gap-4 sm:grid-cols-2">
<Card>
<CardHeader>
<CardTitle className="text-sm">Evidence Quality Score</CardTitle>
<CardDescription>Attribution avgConfidence (avgConfidence)</CardDescription>
</CardHeader>
<CardContent>
...
<CardDescription>Attribution successRate (successRate)</CardDescription>
</CardHeader>
<CardContent>
<div className="mb-2 flex items-baseline justify-between">
...
<section>
<h2 className="mb-3 text-[16px] font-semibold text-[#111111]">Top 5 high effectivenessProduct</h2>
<Card>
<CardContent>
...
<tbody>
{topScores.slice(0, 5).map((s) => (<tr key={s.entityId} className="border-b border-border/30 hover:bg-muted/30">
...
{s.outcomeSuccessRate!= null? `${(s.outcomeSuccessRate * 100).toFixed(1)}%`: "—"}
</td>
</tr>))}
{topScores.length === 0 && (<tr>
...
<CardTitle className="text-sm">Cohort Intelligence · Top 5 Cohorts</CardTitle>
</div>
...
{data.cohorts.slice(0, 5).map((c) => (<li key={c.cohortKey} className="flex items-center justify-between gap-3 py-2.5">
<div className="min-w-0">
<div className="truncate text-[13px] font-semibold text-[#111111]">{c.cohortKey}</div>
<div className="text-[11px] text-[#86868b]">{c.memberCount} members</div>
</div>
...
{c.avgImprovementRate!= null? `${(c.avgImprovementRate * 100).toFixed(1)}%`: "—"}
</div>
</div>
</div>
</li>))}
{data.cohorts.length === 0 && (<li className="py-6 text-center text-[12px] text-[#86868b]">NoCohort IntelligenceData</li>)}
</ul>
</CardContent>
</Card>
...
<CardTitle className="text-sm">HealthBenchmark · Top 5</CardTitle>
</div>
<CardDescription>health_benchmarks · sample_size / median_improvement / median_days_to_improvement</CardDescription>
</CardHeader>
<CardContent>
<ul className="flex flex-col divide-y divide-[#F0EFED]">
{data.benchmarks.slice(0, 5).map((b) => (<li key={b.category} className="flex items-center justify-between gap-3 py-2.5">
<div className="min-w-0">
...
{data.benchmarks.length === 0 && (<li className="py-6 text-center text-[12px] text-[#86868b]">NoHealthBenchmarkData</li>)}
</ul>
</CardContent>
</Card>
</div>
...
return (<div className="flex items-center gap-3.5 rounded-[20px] border border-[rgba(0,0,0,0.05)] bg-white p-5">
<div
...
if (status === "running") {
return (<Badge className="bg-amber-100 text-amber-700">
...
return <Badge variant="outline" className="text-xs">Not running</Badge>
}
...
function buildPolylinePoints(points: TrendPoint[],
field: "evidence_quality_score" | "data_completeness"): string {
...
height="200"
role="img"
aria-label="Flywheel iterationQuality Trends Line Chart"
...
const x = TREND_PADDING_LEFT + (n === 1? innerWidth / 2: (i / (n - 1)) * innerWidth)
return (<g key={`pt-${p.iteration_number}`}>
{p.evidence_quality_score!= null && (<TrendDot
x={x}
value={p.evidence_quality_score}
field="evidence_quality_score"
iterationNumber={p.iteration_number}
/>)}
{p.data_completeness!= null && (<TrendDot
x={x}
...
/>
evidence_quality_score (0-1, originally 0-100 normalized)
</div>
<div className="flex items-center gap-1.5">
<span
className="inline-block size-2.5 rounded-full"
...
const label =
field === "evidence_quality_score"? `evidence_quality_score=${value.toFixed(2)}`: `data_completeness=${value.toFixed(4)}`
...
return (<Fragment key={`row-${it.iteration_number}`}>
<tr
...
{hasError? (<span className="inline-flex items-center gap-1">
{isOpen? (<EmojiIcon name="ChevronDown" className="size-3" />): (<EmojiIcon name="ChevronRight" className="size-3" />)}
#{it.iteration_number}
...
{it.quality_scores.evidence_quality_score!= null? it.quality_scores.evidence_quality_score.toFixed(2): "—"}
</td>
</tr>
{isOpen && hasError && (<tr className="bg-red-50/50">
