...
return (<Link href={`/products/${r.product.id}`}>
<Card
...
{rank === 1 && <Badge className="bg-primary/20 text-primary shrink-0"></Badge>}
...
{(r.product.price_min!= null || r.product.price_max!= null) && (<p className="text-sm font-medium text-primary mt-0.5">
{r.product.price_min!= null && r.product.price_max!= null && r.product.price_min!== r.product.price_max? `$${r.product.price_min} - $${r.product.price_max}`: `$${r.product.price_max?? r.product.price_min}`}
</p>)}
</div>
</div>
...
{hasEvidence? `${filteredBreakdown!.evidence_support![0].data_point} · ${filteredBreakdown!.evidence_support![0].observed_value}`: r.explanation}
</p>
{/* L1: trust level badge */}
{filteredBreakdown?.product_confidence!= null && (<div className="flex items-center gap-1.5 mt-2">
<EmojiIcon name="ShieldCheck" className="size-3 text-primary" />
<span className="text-[10px] text-muted-foreground">
{t("basedOnData", { data: breakdown?.evidence_support?.[0]?.data_point ?? t("dataLabel"), confidence: filteredBreakdown.product_confidence })}
</span>
</div>)}
{/* Timeline-enhanced metrics (Phase 3.5 Shadow Mode) */}
{r.timeline_score!= null && r.timeline_score > 0 && (<div className="mt-3 p-2 rounded-lg bg-muted/50 border border-border/30">
<div className="flex items-center gap-1.5 mb-1.5">
<EmojiIcon name="TrendingUp" className="size-3 text-primary" />
<span className="text-[10px] font-medium text-muted-foreground">{t("timelineRating")}</span>
</div>
<div className="grid grid-cols-2 gap-x-3 gap-y-1 text-[10px]">
<div className="flex justify-between">
<span className="text-muted-foreground">{t("timelineScore")}</span>
<span className="font-semibold tabular-nums">{r.timeline_score.toFixed(0)}</span>
</div>
<div className="flex justify-between">
<span className="text-muted-foreground">{t("reviewScore")}</span>
<span className="font-semibold tabular-nums">{r.review_score?.toFixed(0)?? "—"}</span>
</div>
<div className="flex justify-between">
<span className="text-muted-foreground">{t("stabilityRate")}</span>
<span className="font-semibold tabular-nums">{r.stability_rate?.toFixed(0)}%</span>
</div>
<div className="flex justify-between">
<span className="text-muted-foreground">{t("repurchaseRate")}</span>
<span className="font-semibold tabular-nums">{r.repurchase_rate?.toFixed(0)}%</span>
</div>
{r.soft_stool_risk!= null && r.soft_stool_risk > 0 && (<div className="flex justify-between col-span-2">
...
{expanded && level!== "L1" && filteredBreakdown && (<RecommendationEvidence breakdown={filteredBreakdown} level={level} />)}
...
{expanded? " ": level === "L1"? t("signInToView"): t("viewEvidence")}
{expanded? <EmojiIcon name="ChevronUp" className="size-3" />: <EmojiIcon name="ChevronDown" className="size-3" />}
</Button>

{/* Feedback actions */}
{user && (<span className="inline-flex gap-1 ml-2" data-no-navigate>
<Button
...
{feedback === "accept"? t("accepted"): t("acceptedAction")}
</Button>
<Button
...
{level === "L3" && expanded && (<p className="text-[10px] text-muted-foreground/50 mt-1">
{t("l3FullDataAccess")}
</p>)}
</div>
</CardContent>
