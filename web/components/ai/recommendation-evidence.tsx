...
{ma?.factors && ma.factors.length > 0 && (<div>
<div className="flex items-center gap-1.5 mb-2">
<EmojiIcon name="BarChart3" className="size-3.5 text-muted-foreground" />
<span className="text-xs font-medium text-muted-foreground">{t("statisticsWeight")}</span>
{ma.attribution_type === "statistical_only" && (<Badge variant="outline" className="text-[10px] ml-1">{t("statisticsOnly")}</Badge>)}
</div>
<p className="text-[10px] text-muted-foreground/60 mb-2 italic">
{ma.disclaimer}
</p>
<div className="space-y-2">
{ma.factors.map((f) => (<div key={f.factor} className="flex items-center gap-2">
...
{(breakdown.evidence_support?.length?? 0) > 0 && (<div>
<div className="flex items-center gap-1.5 mb-2">
<EmojiIcon name="Database" className="size-3.5 text-primary" />
<span className="text-xs font-medium text-primary">{t("dataEvidence")}</span>
{level === "L2" && <span className="text-[10px] text-muted-foreground/50 ml-auto">{t("evidenceFiltered")}</span>}
</div>
<div className="space-y-1.5">
{breakdown.evidence_support!.map((ep, i) => (<div key={i} className="flex items-start gap-2 rounded-lg bg-muted/30 p-2 text-xs">
...
{(breakdown.negative_signals?.length?? 0) > 0 && (<div>
<div className="flex items-center gap-1.5 mb-2">
<EmojiIcon name="AlertTriangle" className="size-3.5 text-amber-400" />
<span className="text-xs font-medium text-amber-400">{t("riskSignals")}</span>
{level === "L2" && <span className="text-[10px] text-muted-foreground/50 ml-auto">{t("riskFiltered")}</span>}
</div>
<div className="space-y-1.5">
{breakdown.negative_signals!.map((ns, i) => {
const colors = severityColors[ns.severity]?? severityColors.low
return (<div key={i} className={`rounded-lg border p-2 text-xs ${colors}`}>
<div className="flex items-center gap-2 mb-0.5">
<Badge variant="outline" className="text-[10px]">
{ns.severity === "high"? t("highRisk"): ns.severity === "medium"? t("mediumRisk"): t("lowRisk")}
</Badge>
{ns.actionable && <Badge variant="secondary" className="text-[10px]">{t("actionable")}</Badge>}
</div>
<p className="text-muted-foreground">{ns.signal}</p>
<p className="text-[10px] text-muted-foreground/50 mt-0.5">:{ns.source}</p>
</div>)
})}
</div>
</div>)}
...
{/* Confidence */}
{breakdown.product_confidence!= null && (<div className="flex items-center justify-between text-xs text-muted-foreground pt-1 border-t border-border/40">
<span className="flex items-center gap-1">
<EmojiIcon name="ShieldCheck" className="size-3 text-primary" />{t("dataConfidence")}
</span>
