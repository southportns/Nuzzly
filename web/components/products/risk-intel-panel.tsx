    ...
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-base flex items-center gap-2">
          ...
          Risk Intelligence
          {ri.recent_spike && (
            <Badge variant="destructive" className="text-[10px] ml-2">Anomaly Spike</Badge>
          )}
        </CardTitle>
      </CardHeader>
      <CardContent>
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="text-center rounded-lg bg-muted/30 p-3">
            <p className="text-xs text-muted-foreground mb-1">Stability Grade</p>
            <p className={`text-3xl font-bold ${gradeColor(ri.stability_grade)}`}>{ri.stability_grade}</p>
          </div>
          <div className="text-center rounded-lg bg-muted/30 p-3">
            <p className="text-xs text-muted-foreground mb-1">Risk Index</p>
        ...
        <div className="flex items-center gap-3 mb-4 text-xs">
          ...
          <span className={ri.risk_trend === "rising" ? "text-red-400" : ri.risk_trend === "improving" ? "text-primary" : "text-muted-foreground"}>
            {ri.risk_trend === "rising" ? "Increasing" : ri.risk_trend === "improving" ? "Improving" : "Stable"}
          </span>
          <span className="text-muted-foreground ml-auto flex items-center gap-1">
            <EmojiIcon name="Clock" className="size-3" />
            Time-decayed Risk Score {ri.time_decayed_risk_score?.toFixed(2)}
          </span>
        </div>
          ...
          <div className="space-y-1.5 mb-4">
            {ri.anomalies.rating_drop_detected && (
              <div className="flex items-center gap-2 text-sm text-amber-400 bg-amber-500/5 rounded-lg px-3 py-2">
                ...
                Rating dropped {Math.abs(ri.anomalies.rating_change ?? 0).toFixed(1)} pts in the last 14 days
              </div>
            )}
            {ri.anomalies.stool_spike_detected && (
              <div className="flex items-center gap-2 text-sm text-red-400 bg-red-500/5 rounded-lg px-3 py-2">
                ...
                Soft stool rate spiked {((ri.anomalies.stool_change ?? 0) * 100).toFixed(0)}%
              </div>
            )}
          </div>
        )}
        ...
        {ri.risk_timeline?.length > 0 && (
          <div className="mb-4">
            <p className="text-xs text-muted-foreground mb-2">Risk Timeline (time-decayed)</p>
        ...
        {ri.risk_events?.length > 0 && (
          <div className="space-y-1.5">
            <p className="text-xs text-muted-foreground mb-2">Risk Events (time-weighted)</p>
        ...
        {ri.event_clusters?.length > 0 && (
          <div className="space-y-1.5 mt-3">
            <p className="text-xs text-muted-foreground mb-2">Event Clusters (deduplicated)</p>
            {ri.event_clusters.map((c, i) => (
              <div key={i} className="flex items-center justify-between rounded-lg bg-muted/30 p-2 text-xs">
                ...
                <Badge variant="outline" className="text-[10px] shrink-0">{c.event_count} events</Badge>
              </div>
            ))}
          </div>
        )}
