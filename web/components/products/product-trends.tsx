...
const chartConfig = {
rating: { stroke: "oklch(0.65 0.15 145)", fill: "oklch(0.65 0.15 145 / 0.1)", label: "Overall Rating" },
stool: { stroke: "oklch(0.75 0.15 85)", fill: "oklch(0.75 0.15 85 / 0.1)", label: "Soft Stool Rate" },
repurchase: { stroke: "oklch(0.55 0.2 250)", fill: "oklch(0.55 0.2 250 / 0.1)", label: "Repurchase Rate" },
}
...
const chartData = sorted.map((m) => ({...m,
...
return (<div className="rounded-lg border border-border/40 bg-background/95 backdrop-blur p-3 shadow-xl">
<p className="text-xs font-medium mb-1">{label}</p>
{payload.map((p: TooltipPayloadEntry) => (<div key={p.name} className="flex items-center gap-2 text-xs">
<div className="size-2 rounded-full" style={{ backgroundColor: p.color }} />
<span className="text-muted-foreground">{p.name}</span>
<span className="font-medium tabular-nums">{p.value}{p.name === "Repurchase Rate" || p.name === "Soft Stool Rate"? "%": ""}</span>
</div>))}
</div>)
}
...
<EmojiIcon name="Star" className="size-3" />Overall Rating
</TabsTrigger>
<TabsTrigger value="stool" className="gap-1.5">
<EmojiIcon name="AlertTriangle" className="size-3" />Soft Stool Rate
</TabsTrigger>
<TabsTrigger value="repurchase" className="gap-1.5">
<EmojiIcon name="TrendingUp" className="size-3" />Repurchase Rate
</TabsTrigger>
</TabsList>
...
<Area type="monotone" dataKey="rating" stroke={chartConfig.rating.stroke} fill="url(#ratingFill)" strokeWidth={2} name="Overall Rating" connectNulls />
</AreaChart>
</ResponsiveContainer>
</div>
</TabsContent>
...
<Tooltip content={<CustomTooltip />} />
<Area type="monotone" dataKey="stool_rate" stroke={chartConfig.stool.stroke} fill="url(#stoolFill)" strokeWidth={2} name="Soft Stool Rate" connectNulls />
</AreaChart>
</ResponsiveContainer>
</div>
</TabsContent>
...
<Tooltip content={<CustomTooltip />} />
<Area type="monotone" dataKey="repurchase" stroke={chartConfig.repurchase.stroke} fill="url(#repurchaseFill)" strokeWidth={2} name="Repurchase Rate" connectNulls />
</AreaChart>
</ResponsiveContainer>
</div>
</TabsContent>
