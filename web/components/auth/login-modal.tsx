...
function CloseIcon({ className }: { className?: string }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" className={className}>
      <title>xmark</title>
      <g fill="#212121">
        <path d="M4,14.75c-.192,0-.384-.073-.53-.22-.293-.293-.293-.768,0-1.061L13.47,3.47c.293-.293,.768-.293,1.061,0s.293,.768,0,1.061L4.53,14.53c-.146,.146-.338,.22-.53,.22Z" fill="#212121" />
        <path d="M14,14.75c-.192,0-.384-.073-.53-.22L3.47,4.53c-.293-.293-.293-.768,0-1.061s.768-.293,1.061,0L14.53,13.47c.293,.293,.293,.768,0,1.061-.146,.146-.338,.22-.53,.22Z" fill="#212121" />
      </g>
    </svg>
  )
}
...
const METHODS: { id: LoginMethod; icon: React.FC; bg: string; activeRing: string }[] = [
  { id: "phone",  icon: PhoneIcon,  bg: "#ffffff", activeRing: "#FF7A59" },
  { id: "github", icon: GitHubIcon, bg: "#ffffff", activeRing: "#FF7A59" },
  { id: "apple",  icon: AppleIcon,  bg: "#ffffff", activeRing: "#FF7A59" },
  { id: "wechat", icon: WechatIcon, bg: "#ffffff", activeRing: "#FF7A59" },
  { id: "email",  icon: EmailIcon,  bg: "#ffffff", activeRing: "#FF7A59" },
]
...
export function LoginModal({ open, onOpenChange }: LoginModalProps) {
  ...
  async function handleEmailSubmit(value: string) {
    if (!validateAccount(value)) {
      setError(loginMethod === "phone" ? "请输入有效的 11 位手机号" : "请输入有效的邮箱地址")
      return
    }
    setError("")
    setEmail(value)
  ...
  // ── Icon arc layout ──
  // Places the four login method buttons along a circular arc. With a
  // negative bend the center buttons ride higher and the edge buttons sit
  // lower, forming an upward arch (smile shape).
  const BTN_SIZE = 44
  const BTN_GAP = 20
  const BTN_BEND = -12
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[440px] rounded-[24px] border border-[rgba(0,0,0,0.06)] bg-[#F7F6F3] p-6" showCloseButton={false}>
        <button
          type="button"
          onClick={() => handleOpenChange(false)}
          className="absolute right-4 top-4 flex items-center justify-center size-8 rounded-full text-[#6B6B6B] transition-colors hover:bg-white/80 hover:text-[#111111] z-10"
        >
          <CloseIcon />
        </button>
        ...
        <div className="relative" style={{ top: "10px" }}>
            ...
            step === "email" ? (
              <div className="mt-4">
                <CurvedInput
                  ...
                  placeholder={loginMethod === "phone" ? "请输入手机号" : "your@email.com"}
                  buttonText="下一步"
              ...
              <div className="mt-4">
                <div className="flex items-center justify-center gap-2 mb-2">
                  <button
                    type="button"
                    onClick={handleBack}
                    className="flex items-center justify-center size-8 rounded-full text-[#6B6B6B] hover:text-[#111111] hover:bg-white/80 transition-all"
                    aria-label="返回"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
                      <title>circle-arrow-left</title>
                      <g fill="#212121" strokeLinejoin="miter" strokeLinecap="butt">
          ...
          {error && (
            <p className="mt-4 text-[13px] text-[#ff3b30] text-center">{error}</p>
          )}
        ...
        <p className="mt-4 text-center text-[13px] text-[#6B6B6B]">
          {mode === "login" ? (
            <>
              ...
              <button
                type="button"
                onClick={() => { setMode("signup"); setError(""); setStep("email") }}
            ...
            <>
              ...
              <button
                type="button"
                onClick={() => { setMode("login"); setError(""); setStep("email") }}
                className="text-[#FF7A59] font-medium hover:underline"
              >
                立即登录
              </button>
