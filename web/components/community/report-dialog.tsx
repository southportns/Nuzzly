...
{ value: "violence", label: "Violence / Terrorism" },
...
if (!postId) return
if (!reason.trim()) {
toast.warning("pleaseDescriptionReportreason")
return
}
setSubmitting(true)
...
setSubmitting(false)
if (error) {
toast.error(error.message || "Reportfailed")
return
}
toast.success("ReportalreadySubmit,I s will")
onOpenChange(false)
}
...
<span className="text-[16px] font-semibold text-[#1A1A1A]">Report</span>
<button
...
category === c.value? "border-transparent bg-[#8B5E46] text-white": "border-[#E5E5E5] bg-white text-[#6B6B6B] hover:border-[#8B5E46]"
}`}
>
{c.label}
</button>))}
...
placeholder="pleaseDescriptionReportreason..."
rows={3}
maxLength={500}
