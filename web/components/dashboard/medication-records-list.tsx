...
const tMed = useTranslations("Medication")
const tDisease = useTranslations("Disease")
const locale = useLocale()
const dateLocale = locale === "zh" ? "zh-CN" : "en-US"
const [stoppingId, setStoppingId] = useState<string | null>(null)

const frequencyLabels: Record<string, string> = {
once_daily: tMed("onceDaily"),
twice_daily: tMed("twiceDaily"),
three_times_daily: tMed("threeTimesDaily"),
weekly: tMed("weekly"),
as_needed: tMed("asNeeded"),
}
...
if (!response.ok) throw new Error(tDisease("medicationFailed"))

toast.success(tDisease("medicationStopped"))
window.location.reload()
} catch (error) {
toast.error(tDisease("actionFailed"))
} finally {
setStoppingId(null)
}
}
...
{record.is_ongoing ? (<span className="inline-flex items-center gap-1 text-[10px] text-[#ff9500]">
<EmojiIcon name="Clock" className="size-2.5" />
{tMed("ongoing")}
</span>) : (<span className="inline-flex items-center gap-1 text-[10px] text-[#34c759]">
<EmojiIcon name="CheckCircle" className="size-2.5" />
{tMed("completed")}
</span>)}
</div>
{showStopButton && record.is_ongoing && (<button
onClick={() => handleStop(record.id)}
disabled={isStopping}
className="rounded-full p-2 text-[#ff3b30] hover:bg-[#ff3b30]/10"
>
{isStopping ? (<EmojiIcon name="Loader2" className="size-4 animate-spin" />) : (<EmojiIcon name="StopCircle" className="size-4" />)}
</button>)}
</div>
</div>)
})}
