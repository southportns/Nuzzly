...
const { data, error } = await supabase.from("profiles").select("username, bio, birth_date, avatar_url").eq("id", userId).single()
if (error) {
...
void loadProfile()
}, [userId, initialAvatarUrl, supabase, t])
...
throw new Error(upload.error || t("avatarUploadFailed"))
}
...
if (!trimmedUsername) {
throw new Error(t("usernameEmpty"))
}
...
if (loading) {
return (<div className="flex h-48 items-center justify-center rounded-[20px] border border-[rgba(0,0,0,0.05)] bg-white">
<EmojiIcon name="Loader2" className="size-5 animate-spin text-[#FF7A59]" />
</div>)
}

return (<div className="rounded-[20px] border border-[rgba(0,0,0,0.05)] bg-white shadow-[0_4px_20px_rgba(0,0,0,0.06)]">
{/* Header */}
<div className="flex items-center justify-between border-b border-[rgba(0,0,0,0.05)] px-6 py-4">
<h3 className="text-[16px] font-semibold text-[#111111]">{t("editProfile")}</h3>
<button
type="button"
onClick={onClose}
...
/>
<span className="text-[12px] text-[#6B6B6B]">{t("clickToChangeAvatar")}</span>
</div>

{/* Form fields */}
<div className="min-w-0 flex-1 space-y-5">
<div>
<label className="mb-1.5 block text-[13px] text-[#6B6B6B]">{t("username")}</label>
<input
type="text"
value={form.username ?? ""}
...
<div>
<label className="mb-1.5 block text-[13px] text-[#6B6B6B]">{t("bio")}</label>
<textarea
value={form.bio ?? ""}
...
<div>
<label className="mb-1.5 block text-[13px] text-[#6B6B6B]">{t("birthday")}</label>
<input
type="date"
...
<div>
<label className="mb-1.5 block text-[13px] text-[#6B6B6B]">{t("email")}</label>
<input
type="email"
value={email ?? ""}
disabled
...
/>
<p className="mt-1 text-[11px] text-[#9A9A95]">{t("emailReadonlyHint")}</p>
</div>
</div>

{error && (<div className="rounded-[10px] bg-[#FF3B30]/5 px-3 py-2 text-[12px] text-[#FF3B30]">
{error}
</div>)}
...
>
{saving ? (<span className="flex items-center justify-center gap-1.5">
