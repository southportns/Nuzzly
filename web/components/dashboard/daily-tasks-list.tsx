...
feeding: "categoryFeeding",
water: "categoryFeeding",
litter: "categoryLitter",
walk: "categoryWalk",
bowl_clean: "categoryBowlClean",
deworm: "categoryDeworm",
...
const [completedTasks, setCompletedTasks] = useState<Set<string>>(new Set(logs.filter(l => l.completed).map(l => l.task_id)))
const [loading, setLoading] = useState<string | null>(null)

function taskTitle(task: Task) {
return isEn && task.title_en ? task.title_en : task.title
}

function categoryLabel(category: string) {
const key = categoryLabelKeys[category] || "categoryOther"
return t(key)
}
...
toast.success(t("skippedSuccess"))
} catch (error) {
...
>
{isLoading ? (<EmojiIcon name="Loader2" className="size-5 animate-spin text-[#FF7A59]" />) : isCompleted ? (<EmojiIcon name="CheckCircle2" className="size-5 text-[#34c759]" />) : (<EmojiIcon name="Circle" className="size-5 text-[#D2D1CF] hover:text-[#FF7A59]" />)}
</button>
</div>
</div>)
})}
