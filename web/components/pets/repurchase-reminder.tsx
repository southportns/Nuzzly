...
const { data } = await supabase.from("diet_logs").select("id, food_name, logged_date").eq("pet_id", petId).order("logged_date", { ascending: false }).limit(30)
...
if (error) {
toast.error(error.message || "Actionsfailed")
return
}
toast.success("alreadyCloseReminder")
} catch {
toast.error("Actionsfailed")
}
}

if (!needsReminder) return null

return (<Card className="rounded-[16px] border-[#FF9500]/20 bg-[#FF9500]/5">
...
LastFeedingYes <span className="font-medium text-[#111111]">{daysSinceLast} </span>,not
</p>
