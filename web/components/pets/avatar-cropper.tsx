...
async function getCroppedImg(imageSrc: string,
 pixelCrop: Area,
 outputSize = 512): Promise<File> {
...
export function AvatarCropper({ imageSrc, onConfirm, onCancel }: AvatarCropperProps) {
 ...
 <h3 className="text-[15px] font-semibold text-[#111111]">Adjust Avatar</h3>
 <button
 type="button"
 onClick={onCancel}
 ...
 disabled={cropping}
 className={cn("flex-1 rounded-full bg-[#FF7A59] text-white shadow-[0_4px_12px_rgba(255,122,89,0.25)] hover:bg-[#E86A4A]",
 cropping && "opacity-60")}
 >
 <EmojiIcon name="Check" className="mr-1.5 size-4" />
 {cropping? "Processing...": "Confirm Crop"}
 </Button>
 </div>
 </div>
 </div>)
