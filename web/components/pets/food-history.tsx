...
setLoading(true)
const { data } = await supabase.from("diet_logs").select("id, food_name, food_type, product_id, notes, logged_date, products(name, brand, image_url)").eq("pet_id", petId).order("logged_date", { ascending: false })
setEntries(data || [])
setLoading(false)
}
loadHistory()
...
<Badge variant="outline">{sortedProducts.length + (noProductEntries.length > 0? 1: 0)} products</Badge>
</CardTitle>
</CardHeader>
<CardContent className="space-y-4">
{/* Products with product_id */}
{sortedProducts.length > 0 && (<div className="space-y-3">
...
const duration = diffDays < 30? `${diffDays + 1}`: diffDays < 365? `${Math.floor(diffDays / 30)}months`: `${Math.floor(diffDays / 365)}`

return (<div key={prod.name + firstDate} className={`flex items-start gap-3 p-3 rounded-lg ${isCurrent? "bg-green-50 border border-green-200": "bg-muted/50"}`}>
{isCurrent && <Badge className="bg-green-500 shrink-0 mt-0.5">when </Badge>}
{prod.image_url? (<img
src={prod.image_url}
alt={prod.name}
...
{formatDate(firstDate)} - {formatDate(lastDate)} · {duration} · {prod.entries.length}Record
</p>
</div>
</div>)
})}
...
{noProductEntries.length > 0 && (<div className="space-y-2">
<h4 className="text-sm font-medium text-muted-foreground">OtherDietRecord</h4>
{noProductEntries.slice(0, 10).map((entry) => (<div key={entry.id} className="flex items-start gap-3 p-3 bg-muted/30 rounded-lg">
<div className="w-10 h-10 bg-muted rounded flex items-center justify-center shrink-0">
<EmojiIcon name="Utensils" className="h-5 w-5 text-muted-foreground" />
</div>
<div className="flex-1 min-w-0">
...
{entry.notes && (<p className="text-xs text-muted-foreground mt-1 line-clamp-1">{entry.notes}</p>)}
</div>
</div>))}
</div>)}
...
<p>NofoodusedRecord</p>
<p className="text-sm mt-1">RecordPet Diet,Trackinglong </p>
</div>)}
</CardContent>
</Card>)
}
