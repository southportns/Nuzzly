...
export function DietTrendChart({ petId }: DietTrendChartProps) {
 ...
 const { data: logs } = await supabase.from("diet_logs").select("logged_date, food_name, food_type, notes").eq("pet_id", petId).order("logged_date", { ascending: true }).limit(30)
 ...
 {/* Food Type Distribution */}
 <div>
 <p className="mb-2 text-[12px] text-[#6B6B6B]">Staple Distribution</p>
 <div className="space-y-2">
 {topFood.map(([type, count]) => (<div key={type} className="flex items-center gap-2">
 ...
 {/* Recent Foods */}
 <div className="mt-4">
 <p className="mb-2 text-[12px] text-[#6B6B6B]">Recent Feedings</p>
 <div className="flex flex-wrap gap-1.5">
 {data.slice(-5).reverse().map((d, i) => (<span key={i} className="rounded-full bg-[#F7F6F3] px-2.5 py-1 text-[11px] text-[#6B6B6B]">
 {d.food_name}
 </span>))}
 </div>
 </div>
