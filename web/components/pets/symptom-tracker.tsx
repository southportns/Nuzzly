...
const { data } = await (supabase as any).from("pet_events").select("id, event_time, symptom_code, severity, notes").eq("pet_id", petId).eq("event_type", "symptom_observed").order("event_time", { ascending: false }).limit(20)
...
const trendLabel = last7Days.length > prev7Days.length? "Symptoms Increasing": last7Days.length < prev7Days.length? "Symptoms Decreasing": "Stable"
...
<div>
<p className="text-[13px] text-[#6B6B6B]">Symptoms (Last 7 Days)</p>
<p className="mt-1 text-[24px] font-semibold text-[#111111]">{last7Days.length}<span className="text-[14px] text-[#6B6B6B]">x</span></p>
</div>
...
{/* Symptom Frequency */}
{topSymptoms.length > 0 && (<div className="mb-4">
<p className="mb-2 text-[12px] text-[#6B6B6B]">Symptom Frequency</p>
<div className="space-y-2">
{topSymptoms.map(([code, count]) => (<div key={code} className="flex items-center gap-2">
...
<span className="w-8 text-right text-[12px] text-[#6B6B6B]">{count}x</span>
</div>))}
</div>
</div>)}
...
{/* Recent Symptoms */}
<div>
<p className="mb-2 text-[12px] text-[#6B6B6B]">Recent Symptoms</p>
<div className="space-y-2">
{symptoms.slice(0, 5).map((s) => (<div key={s.id} className="flex items-center justify-between rounded-[10px] bg-[#F7F6F3] p-3">
...
{new Date(s.event_time).toLocaleDateString("en-US")}
</span>
</div>
</div>))}
</div>
