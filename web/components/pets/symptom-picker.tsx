...
const categoryLabels: Record<string, string> = {
  消化: "Digestive System",
  皮肤: "Skin Issues",
  行为: "Behavioral Issues",
  泌尿: "Urinary System",
  体重: "Weight Issues",
  呼吸: "Respiratory System",
  眼部: "Eye Issues",
}
...
disabled={disabled}
>
{selectedSymptom? (<div className="flex items-center gap-2">
...
<CommandList>
<CommandEmpty>
{loading? "Loading...": "notto close Symptom"}
</CommandEmpty>
{Object.entries(groupedSymptoms).map(([cat, catSymptoms]) => (<CommandGroup key={cat} heading={categoryLabels[cat] || cat}>
{catSymptoms.map((symptom) => (<CommandItem
key={symptom.id}
...
{symptom.aliases && Array.isArray(symptom.aliases) && (symptom.aliases as string[]).length > 0 && (<span className="text-xs text-muted-foreground">
some: {(symptom.aliases as string[]).slice(0, 3).join(", ")}
</span>)}
</div>
</CommandItem>))}
</CommandGroup>))}
...
>
{selectedSymptoms.length > 0? `alreadySelect ${selectedSymptoms.length} Symptom`: placeholder}
<EmojiIcon name="ChevronsUpDown" className="ml-2 h-4 w-4 shrink-0 opacity-50" />
</Button>
</PopoverTrigger>
<PopoverContent className="w-[350px] p-0">
<Command>
<CommandInput placeholder="SearchSymptom..." />
<CommandList>
<CommandEmpty>
{loading? "Loading...": "notto close Symptom"}
</CommandEmpty>
...
{/* Selected symptoms */}
{selectedSymptoms.length > 0 && (<div className="flex flex-wrap gap-2">
{selectedSymptoms.map((symptom) => (<Badge
key={symptom.id}
variant="secondary"
