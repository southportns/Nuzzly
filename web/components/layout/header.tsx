...
export function Header() {
  ...
  const leftNavItems = [
    { href: "/", label: t("home") },
    { href: "/ai", label: t("pomiAI") },
    { href: "/products", label: t("products") },
  ]

  const rightNavItems = [
    { href: "/about", label: t("about") },
    { href: "/protection", label: t("protection") },
  ]
  ...
  return (
    <>
      <header className="fixed top-0 z-50 flex w-full justify-center pt-6" style={{ paddingTop: "calc(24px + var(--safe-top))" }}>
        ...
        {/* Capsule navbar - frosted glass + hover float */}
        <nav className="hidden md:flex items-center rounded-full border border-white/25 bg-white/60 backdrop-blur-xl shadow-[0_4px_24px_rgba(0,0,0,0.08),0_1px_2px_rgba(0,0,0,0.04),inset_0_1px_0_rgba(255,255,255,0.6)] py-0 w-[90%] max-w-[1700px] transition-all duration-300 ease-out hover:-translate-y-0.5 hover:shadow-[0_8px_32px_rgba(0,0,0,0.12),0_2px_4px_rgba(0,0,0,0.06),inset_0_1px_0_rgba(255,255,255,0.6)] relative">
          ...
          {/* Left nav - flex:1 40%, buttons evenly distributed */}
          ...
          {/* Logo - fixed width, centered, click to enter community */}
          <Link href="/community" className="group relative flex items-center justify-center px-6 flex-shrink-0 -ml-[3px] transition-transform duration-200 ease-out hover:-translate-y-1 hover:scale-105" title={t("community")}>
            <NuzzlyLogo className="h-16" />
          </Link>

          {/* Right nav - flex:1 40%, buttons evenly distributed */}
          <div className="flex-1 flex items-center justify-between gap-1 px-[30px]">
            <Link
              href="/about"
              className={cn(
            ...
            {user ? (
              ...
              <Link
                href="/login"
                className="px-6 py-2 w-28 rounded-full text-[15px] font-semibold text-white bg-gradient-to-r from-[#8B5E46] to-[#A67D65] shadow-[0_2px_8px_rgba(139,94,70,0.25)] transition-all hover:shadow-[0_4px_12px_rgba(139,94,70,0.35)] text-center whitespace-nowrap no-underline"
              >
                {t("signIn")}
              </Link>
            )}
        ...
        <div className="flex w-full items-center justify-between px-4 md:hidden">
          <Link href="/community" className="flex items-center" title={t("community")}>
          ...
          <nav className="mx-4 flex flex-col gap-2 p-3 rounded-[28px] border border-white/20 bg-white/70 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.1),inset_0_1px_0_rgba(255,255,255,0.5)]">
            ...
            {user ? (
              ...
              <Link
                href="/login"
                onClick={() => setMobileOpen(false)}
                className="mt-2 flex h-[48px] w-full items-center justify-center rounded-2xl bg-gradient-to-r from-[#8B5E46] to-[#A67D65] px-6 text-[14px] font-semibold text-white shadow-[0_4px_12px_rgba(139,94,70,0.3)] no-underline"
              >
                {t("signIn")}
              </Link>
            )}
