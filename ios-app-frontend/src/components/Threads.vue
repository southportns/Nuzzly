<template>
  <div ref="containerRef" class="threads-container" v-bind="$attrs"></div>
</template>
...
const props = defineProps({
  color: { type: [Array, String], default: () => [1, 1, 1] },
...
function parseColor(c) {
  ...
  const m = String(c).match(/rgba?\(\s*([\d.]+)\s*,\s*([\d.]+)\s*,\s*([\d.]+)\s*/)
...
function updateUniforms() {
  if (!program) return
  program.uniforms.uColor.value.set(...parseColor(props.color))
  program.uniforms.uAmplitude.value = props.amplitude
  program.uniforms.uDistance.value = props.distance
}

watch(() => [props.color, props.amplitude, props.distance], updateUniforms, { deep: true })
...
onMounted(() => {
  ...
  program = new Program(gl, {
    ...
    uniforms: {
      ...
      uColor: { value: new Color(...parseColor(props.color)) },
