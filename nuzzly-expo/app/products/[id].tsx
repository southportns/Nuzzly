...
const SEGS = [{ value: 'reviews', label: 'Long-termFeedback' },
 { value: 'ingredients', label: 'Ingredients' },
 { value: 'timeline', label: 'Time' },
 { value: 'risks', label: 'RiskRecord' },];
...
export default function Product DetailsScreen() {
 ...
 return s === 'cats'? 'Catuse ': s === 'dogs'? 'Doguse ': 'CatDogGeneral';
 }, [product]);

 const ageLabel = useMemo(() => {
 const a = product?.applicable_age;
 return a === 'kitten'? 'Kitten': a === 'adult'? ' Y': a === 'senior'? 'Senior': 'Age';
 }, [product]);
 ...
 latestMetric.stool_issue_rate!= null? `${(Number(latestMetric.stool_issue_rate) * 100).toFixed(1)}%`: '--',
 [latestMetric]);
 const stoolRatePct = Number(latestMetric.stool_issue_rate?? 0) * 100;
 const repurchaseRateLabel = useMemo(() =>
 latestMetric.repurchase_rate!= null? `${(Number(latestMetric.repurchase_rate) * 100).toFixed(0)}%`: '--',
 [latestMetric]);
 const repurchaseRatePct = Number(latestMetric.repurchase_rate?? 0) * 100;

 const currentVersion = useMemo(() => versions.find((v) => v.is_current) || null,
 [versions]);
 ...
 const sorted = [...riskevents].sort((a, b) => new Date(b.event_date || 0).getTime() - new Date(a.event_date || 0).getTime());
 ...
 const tags = [{ label: 'Soft Stool', value: m.stool_issue_rate, priority: 'SSS', color: '#E85D4A' },
 { label: 'next', value: m.black_chin_rate, priority: 'SSS', color: '#E85D4A' },
 { label: 'Vomiting', value: m.vomit_rate, priority: 'SSS', color: '#E85D4A' },
 { label: '', value: m.tear_stain_rate, priority: 'SS', color: '#E8A87C' },
 { label: 'Palatability', value: m.average_rating!= null? 1 - Number(m.average_rating) / 5: null, priority: 'SS', color: '#E8A87C' },
 { label: 'Long-term Stability', value: m.long_term_stability_score!= null? 1 - Number(m.long_term_stability_score) / 100: null, priority: 'SS', color: '#E8A87C' },
 { label: '', value: m.dise_rate, priority: 'SS', color: '#E8A87C' },
 { label: 'Shedding', value: m.shedding_rate, priority: 'S', color: '#6B7B6B' },
 { label: 'Good', value: m.coat_improve_rate!= null? 1 - Number(m.coat_improve_rate): null, priority: 'S', color: '#6B7B6B' },
 { label: '', value: m.repurchase_rate!= null? 1 - Number(m.repurchase_rate): null, priority: 'S', color: '#6B7B6B' },].filter((t) => t.value!= null);
 ...
 return data.map((m) => ({
 month: m.date? new Date(m.date).toLocaleDateString('en-SG', { month: 'short' }): '--',
 ...
 if (!id) return;
 (async () => {
 const [p, ing, ver, tags, met, risks, bked] = await Promise.all([fetchProduct(id),
 fetchIngredients(id),
 fetchVersions(id),
 fetchProductTags(id),
 fetchMetrics(id),
 ...
 const formatDate = (d?: string) => (d? new Date(d).toLocaleDateString('en-SG'): '');
 ...
 const renderStars = (rating: number, size = 14) => (<View style={styles.stars}>
 {[1, 2, 3, 4, 5].map((i) => (<Ionicons
 key={i}
 ...
 if (loading) {
 return (<View style={[styles.loadingState, { paddingTop: insets.top }]}>
 ...
 if (!product) {
 return (<View style={{ paddingTop: insets.top, flex: 1, backgroundColor: colors.bg }}>
 <EmptyState icon="🔍" title="ProductNot " actionText="Backlast1" onAction={() => router.back()} />
 </View>);
 }
 ...
 {product.image_url? (<Image source={{ uri: product.image_url }} style={styles.heroImg} />): (<Text style={styles.heroPlaceholder}>{product.brand?.slice(0, 1) || '🐾'}</Text>)}
 </View>
 <View style={styles.heroInfo}>
 <Text style={styles.heroBrand}>
 {product.product_categories?.name || 'Product'} · {product.brand}
 </Text>
 ...
 {product.price_min? (<Text style={[styles.tag, styles.tagPrice]}>
 ${Number(product.price_min)}
 {product.price_max && product.price_max!== product.price_min? `-${Number(product.price_max)}`: ''}
 </Text>): null}
 {product.origin_country? <Text style={styles.tag}>Origin: {product.origin_country}</Text>: null}
 ...
 <Text style={styles.reviewBtnText}>Submitmake use Feedback</Text>
 </TouchableOpacity>
 </View>
 </View>

 {productTags.length? (<View style={styles.tagCloud}>
 {productTags.map((t) => (<Text
 key={t.id}
 ...
 <View style={styles.statCard}>
 <Text style={styles.statLabel}>Stool Safety Rate</Text>
 ...
 <View style={styles.statCard}>
 <Text style={styles.statLabel}>Repurchase Rate</Text>
 ...
 <View style={styles.statCard}>
 <Text style={styles.statLabel}>TransparencyScore</Text>
 <View style={styles.statValueRow}>
 ...
 {trendData.length > 1? (<View style={styles.trendSection}>
 <Text style={styles.sectionTitle}></Text>
 <View style={styles.trendCard}>
 <View style={styles.trendRow}>
 <Text style={styles.trendLabel}>Score</Text>
 <View style={styles.trendBars}>
 {trendData.map((d, i) => (<View key={i} style={styles.trendBarWrap}>
 <View
 ...
 <View style={styles.trendRow}>
 <Text style={styles.trendLabel}>Repurchase Rate</Text>
 <View style={styles.trendBars}>
 {trendData.map((d, i) => (<View key={i} style={styles.trendBarWrap}>
 <View
 style={[styles.trendBar,
 ...
 {monitorTags.length? (<View style={styles.monitorSection}>
 <Text style={styles.sectionTitle}>Long-term</Text>
 <View style={styles.monitorChips}>
 {monitorTags.map((m) => (<View key={m.label} style={styles.monitorChip}>
 ...
 {riskSummary? (<View style={styles.riskIntelSection}>
 ...
 <Text style={styles.riskIntelLabel}>Riskevent</Text>
 <Text
 style={[styles.riskIntelValue,
 ...
 <View style={styles.riskIntelItem}>
 <Text style={styles.riskIntelLabel}>Severity/premiumRisk</Text>
 <Text style={[styles.riskIntelValue, riskSummary.highCount > 0? styles.danger: styles.ok]}>
 {riskSummary.highCount}
 </Text>
 </View>
 <View style={styles.riskIntelItem}>
 <Text style={styles.riskIntelLabel}> </Text>
 <Text style={[styles.riskIntelValue, styles.ok]}>{riskSummary.resolved}</Text>
 </View>
 </View>
 {riskSummary.recentevent? (<View style={styles.riskIntelRecent}>
 <Text style={styles.riskIntelRecentLabel}>mostevent: </Text>
 ...
 {t.value === 'risks'? (<Ionicons name="warning-outline" size={13} color={active? colors.primary: colors.muted} />): null}
 </TouchableOpacity>);
 })}
 </View>

 <View style={styles.segContent}>
 {seg === 'reviews'? (reviewsLoading? (<View style={styles.reviewSkeletonList}>
 {[1, 2, 3].map((i) => (<View key={i} style={styles.reviewSkeleton}>
 ...
 </View>): reviews.length? (<View style={styles.reviewList}>
 {(reviews as Review[]).map((r) => (<View key={r.id} style={styles.reviewCard}>
 <View style={styles.reviewHead}>
 <View>
 <Text style={styles.reviewUser}>{r.profiles?.display_name || 'Anonymous'}</Text>
 <Text style={styles.reviewPet}>
 {r.pets?.name} · {r.pets?.breed || 'Unknown Breed'} · {' '}
 {r.pets?.stomach_health === 'sensitive'? 'Sensitive Stomach': 'StomachNormal'}
 </Text>
 </View>
 ...
 </View>): (<Text style={styles.emptyMini}>NoneReview. 1 ReviewthisProduct!</Text>)): seg === 'ingredients'? (<View>
 {ingredients.length? (<View style={styles.ingredientList}>
 {ingredients.map((ing) => (<View key={ing.id} style={styles.ingredientRow}>
 <View style={styles.ingLeft}>
 <Text style={styles.ingName}>{ing.ingredient_name}</Text>
 {ing.allergen_risk && ing.allergen_risk.length? (<Text style={styles.ingAllergen}>AllergyRisk</Text>): null}
 </View>
 ...
 </View>): (<Text style={styles.emptyMini}>NoneIngredientsData</Text>)}
 {currentVersion?.nutrition_snapshot && Object.keys(currentVersion.nutrition_snapshot).length? (<View style={styles.nutritionSection}>
 <Text style={styles.sectionTitle}>Nutrition</Text>
 <View style={styles.nutritionGrid}>
 {Object.entries(currentVersion.nutrition_snapshot).map(([key, value]) => (<View key={key} style={styles.nutritionCard}>
 <Text style={styles.nutritionValue}>{value}%</Text>
 <Text style={styles.nutritionKey}>{key}</Text>
 </View>))}
 </View>
 </View>): null}
 </View>): seg === 'timeline'? (versions.length? (<View style={styles.versionTimeline}>
 {versions.map((v, i) => (<View key={v.id} style={styles.versionItem}>
 ...
 {v.is_current? <Text style={styles.versionCurrentTag}>ago</Text>: null}
 </View>
 {v.effective_date? (<Text style={styles.versionDate}>
 ...
 </View>): (<Text style={styles.emptyMini}>NoneVersionRecord</Text>)): (riskevents.length? (<View style={styles.riskList}>
 {riskevents.map((ev) => (<View key={ev.id} style={styles.riskCard}>
 <View style={styles.riskHead}>
 <Text style={styles.riskTitle}>{ev.title}</Text>
 <Text
 style={[styles.riskSev,
 ev.severity === 'critical' || ev.severity === 'high'? styles.riskSevpremium: ev.severity === 'medium'? styles.riskSevMedium: styles.riskSevLow,]}
 >
 ...
 <Text style={styles.riskMetaText}>{ev.report_count || 0} records</Text>
 {ev.resolved? <Text style={styles.riskResolved}> </Text>: null}
 </View>
 </View>))}
 </View>): (<Text style={styles.emptyMini}>NoneRiskRecord</Text>))}
 </View>
 </ScrollView>);
}
