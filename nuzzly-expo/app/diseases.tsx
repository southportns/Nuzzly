...
export default function DiseasesScreen() {
  ...
  const stats = useMemo(() => {
    const active = diseaseRecords.filter(
      (d) => d.status === 'active' || d.status === 'under_treatment'
    );
  ...
  return (
    <View style={[styles.shell, { paddingTop: insets.top }]}>
      <PageHeader title="疾病记录" actionText="添加" onAction={() => setShowSheet(true)} />
        ...
        {!loading && (
          <View style={styles.statsRow}>
            ...
            <View style={styles.statCard}>
              ...
              <Text style={styles.statLabel}>慢性病</Text>
            </View>
            <View style={styles.statCard}>
              <Text style={[styles.statValue, { color: '#34c759' }]}>{stats.resolved.length}</Text>
              <Text style={styles.statLabel}>已康复</Text>
            </View>
          </View>
        )}

        {!loading && diseaseRecords.length === 0 ? (
          <EmptyState icon="🏥" title="暂无疾病记录" />
        ) : (
