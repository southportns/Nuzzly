...
export default function DailyTasksScreen() {
 ...
 if (task.frequency === 'custom_days') return `every ${task.custom_days || 1} days`;
 ...
 Alert.alert('ActionsFailed', e.message || 'ActionsFailed');
 }
 }
 ...
 const renderTask = (task: DailyTask & { completed?: boolean }, completedSection = false) => (<TouchableOpacity
 key={task.id}
 activeOpacity={0.8}
 ...
 {task.weight}pts · {freqLabel(task)}
 ...
 <PageHeader title="Task Management" actionText="Add" onAction={() => setShowAddSheet(true)} showBack />

 {pets.length > 1 && (<ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.petTabs}>
 {pets.map((pet) => {
 ...
 {!loading && (<View style={styles.scoreOverview}>
 ...
 <Text style={[styles.scoreUnit, { color: scoreColor }]}>pts</Text>
 </View>
 <View style={styles.scoreDetail}>
 <Text style={styles.scoreTitle}>TodayDone </Text>
 ...
 {pendingTasks.length === 0 && <Text style={styles.emptyHint}>🎉 All done! Great job!</Text>}
 {pendingTasks.map((task) => renderTask(task))}
 </View>

 {completedTasks.length > 0 && (<View style={styles.section}>
 <View style={styles.sectionHeader}>
 <Text style={[styles.sectionTitle, styles.sectionTitleDone]}>Completed</Text>
 ...
 {showTemplates && (<View style={styles.templateGrid}>
 {availableTemplates.map((tpl) => (<TouchableOpacity
 key={tpl.title}
 activeOpacity={0.8}
 onPress={() => {
 ...
 {!availableTemplates.length && <Text style={styles.emptyHint}>No templates used</Text>}
 </View>)}
 </View>
 </>)}
 </ScrollView>
 ...
 <Text style={styles.cancelBtnText}>BackSelect</Text>
 </TouchableOpacity>
 <TouchableOpacity
 activeOpacity={0.8}
 ...
 >
 {!customForm.title? (<View style={styles.quickTemplates}>
 {builtinTemplates.map((tpl) => (<TouchableOpacity key={tpl.title} activeOpacity={0.8} onPress={() => fillFromTemplate(tpl)} style={styles.quickTplRow}>
 <Text>{tpl.icon}</Text>
 ...
 <Text>✏</Text>
 <Text style={styles.quickTplCustomText}>Custom Task</Text>
 </TouchableOpacity>
 </View>): (<>
 ...
 placeholder="e.g., Clean litter box"
 ...
 {COMMON_ICONS.map((icon) => (<TouchableOpacity
 key={icon}
 activeOpacity={0.8}
 ...
 {FREQUENCY_OPTIONS.map((o) => (<TouchableOpacity
 key={o.value}
 activeOpacity={0.8}
 ...
 {customForm.frequency === 'custom_days' && (<View style={[styles.formGroup, styles.half]}>
 <Text style={styles.formLabel}>Interval (days)</Text>
 <TextIn
 ...
 <View style={styles.formRow}>
 <View style={[styles.formGroup, styles.half]}>
 <Text style={styles.formLabel}>Weight (pts)</Text>
 ...
 onClose={() => setShowConfirm Delete(false)}
 title="Confirm Delete"
 footer={
 <>
 <TouchableOpacity activeOpacity={0.8} onPress={() => setShowConfirm Delete(false)} style={styles.cancelBtn}>
 ...
 Are you sure you want to deleteTask "{deletingTask?.title}"?
 </Text>
 </BottomSheet>
 </View>);
}
