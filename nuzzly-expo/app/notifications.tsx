...
export default function NotificationsScreen() {
 ...
 return d.toLocaleDateString('en-SG', { month: '2-digit', day: '2-digit' });
 };
 ...
 <Text style={styles.pageTitle}>NotificationsCenter</Text>
 {unreadCount > 0? (<TouchableOpacity onPress={markAllRead} activeOpacity={0.7}>
 <Text style={styles.markAll}>AllRead</Text>
 </TouchableOpacity>): (<Text style={[styles.markAll, styles.markAllDone]}> Allread</Text>)}
 </View>

 <View style={styles.subtitle}>
 <Text style={styles.subtitleText}>
 {unreadCount > 0? `still ${unreadCount} recordsUnread`: 'all NotificationsRead'}
 </Text>
 <View style={styles.totalBadge}>
 <Text style={styles.totalText}> {notifications.length} records</Text>
 </View>
 </View>

 <ScrollView contentContainerStyle={styles.body}>
 {loading? (<View style={styles.list}>
 {[1, 2, 3, 4].map((i) => (<View key={i} style={styles.notifCardSkeleton}>
 ...
 </View>): notifications.length > 0? (<View style={styles.list}>
 {notifications.map((n) => {
 ...
 icon="🔔"
 title="NoneNotifications"
 description=" new TrackingReminderor Notifications, this "
 />)}
 </ScrollView>
 </View>);
}
