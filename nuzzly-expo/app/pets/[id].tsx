...
export default function PetDetailScreen() {
 ...
 setLoading(true);
 const { data, error } = await supabase.from('pets').select('id, name, species, breed, age_Y, age_months, gender, weight_kg, neutered, stomach_health, photo_url').eq('id', id).single();
 if (error ||!data) {
 setPet(null);
 } else {
 const p = data as Pet;
 setPet(p);
 setEditForm({
 age_Y: p.age_Y != null? String(p.age_Y): '',
 age_months: p.age_months != null? String(p.age_months): '',
 weight_kg: p.weight_kg!= null? String(Math.round(p.weight_kg * 100) / 100): '',
 ...
 const [dietRes, healthRes, allergyRes] = await Promise.all([supabase.from('diet_logs').select('id, food_type, food_name, notes, logged_date, created_at').eq('pet_id', id).order('created_at', { ascending: false }).limit(20),
 supabase.from('health_records').select('id, record_type, record_time, diagnosis, medication_name, notes').eq('pet_id', id).order('record_time', { ascending: false }).limit(20),
 supabase.from('pet_allergies').select('id, allergen, severity').eq('pet_id', id),]);
 setDietLogs((dietRes.data || []) as DietLog[]);
 setHealth Recordss(((healthRes.data || []) as Health Records[]).filter((r) => r.record_type!== 'weight'));
 ...
 Alert.alert('Save Failed', e.message || 'Save Failed');
 } finally {
 setSaving(false);
 }
 }
 ...
 if (!pet) return;
 Alert.alert(`Are you sure you want to delete ${pet.name} profile?`,
 ...
 const renderRadio = (label: string, value: boolean) => (<TouchableOpacity
 activeOpacity={0.8}
 ...
 <PageHeader title="PetDetails" showBack />
 ...
 <PageHeader title="PetDetails" showBack />
 <View style={styles.loadingBody}>
 <Text style={{ color: colors.muted }}>Not findto Pet</Text>
 </View>
 </View>);
 }
 ...
 {pet.photo_url? (<Image source={{ uri: pet.photo_url }} style={styles.avatarImg} />): (<Text style={styles.avatarEmoji}>{SPECIES_EMOJI[pet.species] || '🐾'}</Text>)}
 </View>
 <Text style={styles.petNameLg}>{pet.name}</Text>
 <Text style={styles.petMetaLg}>
 {pet.breed || 'Unknown Breed'} · {pet.gender === 'male'? 'Male': pet.gender === 'female'? 'Female': 'Unknown'}
 </Text>
 </View>

 <View style={styles.glassCard}>
 <View style={styles.cardHeader}>
 <Text style={styles.cardTitle}>📋 Generalbelieve</Text>
 </View>
 {!editing? (<View style={styles.infoGrid}>
 <View style={styles.infoItem}>
 <Text style={styles.infoLabel}>Age</Text>
 <Text style={styles.infoValue}>
 {pet.age_Y || 0}Y old{pet.age_months || 0}M
 </Text>
 </View>
 <View style={styles.infoItem}>
 ...
 <View style={styles.infoItem}>
 <Text style={styles.infoLabel}>Stomach</Text>
 ...
 label="Age(Y old)"
 type="number"
 value={editForm.age_Y}
 onChange={(v: string) => setEditForm((f) => ({...f, age_Y: v }))}
 placeholder="0"
 />
 <FormField label="M" type="number" value={editForm.age_months} onChange={(v: string) => setEditForm((f) => ({...f, age_months: v }))} placeholder="0" />
 </View>
 <FormField
 label="Weight(kg)"
 type="number"
 value={editForm.weight_kg}
 onChange={(v: string) => setEditForm((f) => ({...f, weight_kg: v }))}
 placeholder="if: 4.8"
 />
 <FormField label="Neutered">
 <View style={styles.radioGroup}>
 {renderRadio(' Neutered', true)}
 {renderRadio('Not Neutered', false)}
 </View>
 </FormField>
 ...
 {saving? <ActivityIndicator color="#fff" />: <Text style={styles.saveBtnText}>SaveModify</Text>}
 </TouchableOpacity>
 </View>)}
 </View>
 ...
 <View style={styles.glassCard}>
 <View style={styles.cardHeader}>
 <Text style={styles.cardTitle}>🍖 Diet Logs</Text>
 <Text style={styles.cardBadge}>{dietLogs.length} records</Text>
 </View>
 {dietLogs.slice(0, 5).map((log) => (<View key={log.id} style={styles.recordItem}>
 ...
 {!dietLogs.length && <Text style={styles.emptyMini}>No diet records</Text>}
 </View>

 <View style={styles.glassCard}>
 <View style={styles.cardHeader}>
 <Text style={styles.cardTitle}>💊 Health Records</Text>
 <Text style={styles.cardBadge}>{healthRecords.length} records</Text>
 </View>
 {healthRecords.slice(0, 5).map((r) => (<View key={r.id} style={styles.recordItem}>
 ...
 {!healthRecords.length && <Text style={styles.emptyMini}>NoneHealth Records</Text>}
 </View>
 ...
 {allergies.map((a) => (<Text key={a.id} style={styles.allergyTag}>
 {a.allergen} · {SEVERITY_LABEL[a.severity] || ''}
 </Text>))}
 {!allergies.length && (<Text style={[styles.allergyTag, styles.allergyTagEmpty]}>NoneAllergyRecord</Text>)}
 </View>
 </View>

 <View style={styles.dangerZone}>
 <TouchableOpacity activeOpacity={0.8} onPress={handleDelete} style={styles.dangerBtn}>
 <Text style={styles.dangerBtnText}>DeletePet profiles</Text>
 </TouchableOpacity>
 </View>
 </ScrollView>
 </View>);
