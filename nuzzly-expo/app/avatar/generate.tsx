...
export default function AvatarGenerateScreen() {
 ...
 if (!session) throw new Error('Not Sign In');
 ...
 const { data: uploadData, error: uploadErr } = await supabase.storage.from('pet-avatars').upload(fileName, blob, { cacheControl: '3600', upsert: false });
 ...
 alert(err.message || 'UploadFailed, Please try again later');
 } finally {
 setGenerating(false);
 }
 };

 return (<View style={[styles.container, { paddingTop: insets.top, paddingBottom: insets.bottom + spacing.md }]}>
 ...
 3D{'\n'}
 <Text style={styles.highlight}>AICreateexclusivePet</Text>
 </Text>
 </View>

 <TouchableOpacity style={styles.previewCard} onPress={triggerUpload} activeOpacity={0.9}>
 {avatarUrl? (<View style={styles.previewImage}>
 <Image source={{ uri: avatarUrl }} style={styles.generatedImg} />
 <Text style={styles.previewHint}> </Text>
 </View>): photoPreview? (<View style={styles.previewImage}>
 <Image source={{ uri: photoPreview }} style={styles.generatedImg} />
 <Text style={styles.previewHint}> morePhoto</Text>
 </View>): (<View style={styles.previewPlaceholder}>
 <Ionicons name="camera-outline" size={48} color={colors.muted} />
 <Text style={styles.placeholderText}>UploadPetPhoto</Text>
 </View>)}
 </TouchableOpacity>
 ...
 {generating? 'Medium...': '3D'}
 </Text>
 </TouchableOpacity>

 {avatarUrl? (<View style={styles.successHint}>
 <Ionicons name="checkmark" size={16} color="#27AE60" />
 <Text style={styles.successHintText}> and Save</Text>
 </View>): null}
 </View>

 <View style={styles.finishArea}>
 {avatarUrl? (<TouchableOpacity style={styles.finishBtn} onPress={goToCreatePet} activeOpacity={0.8}>
 <Text style={styles.finishBtnText}>Next: CreatePet profiles</Text>
 </TouchableOpacity>): (<TouchableOpacity style={styles.skipBtn} onPress={goToCreatePet} activeOpacity={0.8}>
 <Text style={styles.skipBtnText}>Skip, Createprofile</Text>
 </TouchableOpacity>)}
 </View>
 </ScrollView>
 </View>);
