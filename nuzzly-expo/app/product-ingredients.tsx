...
export default function ProductIngredientsScreen() {
 ...
 {[1, 2, 3, 4, 5].map((i) => (<View key={i} style={styles.ingredientSkeleton}>
 <View style={[styles.shimmerLine, { width: '60%' }]} />
 <View style={[styles.shimmerLine, { width: '40%' }]} />
 </View>))}
 </View>): ingredients.length === 0? (<EmptyState icon="📊" title="NoneIngredientsData" />): (<View style={styles.content}>
 <View style={styles.nutritionSummary}>
 <Text style={styles.summaryTitle}>NutritionDigest</Text>
 ...
 {summary.hasNovelProtein && (<Text style={[styles.tag, styles.tagNovel]}>new</Text>)}
 {summary.isGrainFree && (<Text style={[styles.tag, styles.tagGrainFree]}>No </Text>)}
 </View>
 </View>

 <Text style={styles.sectionTitle}>IngredientsList ({ingredients.length})</Text>
 <View style={styles.ingredientList}>
 {ingredients.map((ing) => (<View key={ing.id} style={styles.ingredientItem}>
 <View
 style={[styles.ingColor,
