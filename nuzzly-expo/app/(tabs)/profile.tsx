...
import { useHealth Recordss, Health Records, Allergy } from '../../src/hooks/useHealth Recordss';
...
export default function profile() {
 ...
 rawHealth.slice(0, 3).map((r: Health Records) => ({
 id: r.id,
 ...
 useEffect(() => {
 if (selectedPet) {
 fetchHealth Recordss(selectedPet);
 fetchDietLogs(selectedPet);
 fetchAllergies(selectedPet);
 }
 }, [selectedPet, fetchHealth Recordss, fetchDietLogs, fetchAllergies]);
 ...
 show('Weight recorded', 'success');
 setNewWeight('');
 setShowWeightForm(false);
 fetchHealth Recordss(selectedPet);
 } catch (e: any) {
 show(e.message || 'Record Failed', 'error');
 } finally {
 ...
 Alert.alert('Confirm Delete', `Are you sure you want to delete ${label} 's weight record (${value} kg)?`, [{ text: 'Cancel', style: 'cancel' },
 {
 text: 'Delete',
 style: 'destructive',
 onPress: async () => {
 try {
 await deleteHealth Records(id);
 show('Deleted', 'success');
 if (selectedPet) fetchHealth Recordss(selectedPet);
 } catch (e: any) {
 ...
 show('Allergy added', 'success');
 setNewAllergen('');
 setNewSeverity('mild');
 setNewConfirmed(false);
 setShowAllergyForm(false);
 } catch (e: any) {
 show(e.message || 'Add Failed', 'error');
 }
 }
 ...
 Alert.alert('Confirm Delete', 'the AllergyConfirmed, Are you sure you want to delete?', [{ text: 'Cancel', onPress: () => resolve(false) },
 ...
 {avatarUrl? (<Image source={{ uri: avatarUrl }} style={styles.profileAvatarImg} />): (<Image source={require('../../assets/images/mqpyqgao-logo.png')} style={styles.profileAvatarImg} />)}
 </View>
 ...
 <Text style={styles.statLabel}>Followingers</Text>
 </TouchableOpacity>
 <TouchableOpacity activeOpacity={0.7} style={styles.statItem}>
 <Text style={styles.statNum}>0</Text>
 ...
 <Text style={styles.actionBtnTextPrimary}>Edit profile</Text>
 </TouchableOpacity>
 <TouchableOpacity
 activeOpacity={0.9}
 ...
 >
 {pets.map((pet: any) => (<TouchableOpacity
 key={pet.id}
 activeOpacity={0.8}
 ...
 <Text style={[styles.petChipName, { color: colors.muted }]}>Add</Text>
 </TouchableOpacity>
 </ScrollView>
 ...
 <Text style={styles.glassCardTitleText}>Diet Logs</Text>
 </View>
 <TouchableOpacity activeOpacity={0.7} style={styles.glassCardMore}>
 <Text style={styles.glassCardMoreText}>All</Text>
 </TouchableOpacity>
 </View>
 {dietLogs.length? (dietLogs.map((l: any) => (<View key={l.id} style={styles.recordItem}>
 ...
 </View>))): (<View style={styles.recordItem}>
 <View style={styles.recordContent}>
 <Text style={[styles.recordName, { color: colors.muted }]}>No diet records</Text>
 </View>
 </View>)}
 </View>
 ...
 <Text style={styles.glassCardTitleText}>Health Records</Text>
 </View>
 <View style={styles.glassCardBadge}>
 <Text style={styles.glassCardBadgeText}>{rawHealth.length} records</Text>
 </View>
 </View>
 {healthRecords.length? (healthRecords.map((r: any) => (<View key={r.id} style={styles.recordItem}>
 ...
 <View style={styles.recordContent}>
 <Text style={[styles.recordName, { color: colors.muted }]}>NoneHealth Records</Text>
 </View>
 </View>)}
 </View>
 ...
 <Text style={styles.glassCardTitleText}>Weight Trend</Text>
 </View>
 <TouchableOpacity activeOpacity={0.7} onPress={() => setShowWeightForm((v) =>!v)}>
 <Text style={styles.addBtnText}>{showWeightForm? 'Collapse': '+ Record'}</Text>
 </TouchableOpacity>
 </View>
 {showWeightForm && (<View style={styles.weightForm}>
 <TextIn
 style={styles.weightIn}
 value={newWeight}
 ...
 >
 <Text style={styles.weightSubmitText}>{weightSaving? 'Recording...': 'Record'}</Text>
 </TouchableOpacity>
 </View>)}
 ...
 <Text style={styles.weightMiniLabel}>Latest Weight</Text>
 </View>
 <View style={styles.weightBarChart}>
 {weightChartData.map((item) => (<TouchableOpacity
 key={item.id}
 activeOpacity={0.8}
 ...
 {!weightChartData.length && (<Text style={{ fontSize: 12, color: colors.muted }}>No data yet</Text>)}
 </View>
 </View>
 </View>
 ...
 <Text style={styles.glassCardTitleText}>Allergy Info</Text>
 </View>
 {!showAllergyForm && (<TouchableOpacity activeOpacity={0.7} onPress={() => setShowAllergyForm(true)}>
 <Text style={styles.addBtnText}>+ Add</Text>
 </TouchableOpacity>)}
 </View>
 <View style={styles.allergyTags}>
 {allergyTags.map((a: any) => (<View key={a.id} style={styles.allergyTag}>
 ...
 {!allergyTags.length &&!showAllergyForm && (<View style={[styles.allergyTag, styles.allergyTagEmpty]}>
 <Text style={{ color: colors.muted, fontSize: 12 }}>None</Text>
 </View>)}
 </View>
 {showAllergyForm && (<View style={styles.allergyForm}>
 <TextIn
 style={styles.allergyIn}
 value={newAllergen}
 onChangeText={setNewAllergen}
 placeholder="Allergen name, e.g., chicken, grain"
 placeholderTextColor={colors.muted}
 />
 ...
 <Text style={styles.allergyCheckText}>Confirmed</Text>
 </TouchableOpacity>
 </View>
 ...
 >
 <Text style={styles.allergySubmitText}>Add</Text>
 </TouchableOpacity>
 <TouchableOpacity
 activeOpacity={0.8}
 ...
 >
 <Text style={styles.allergyCancelText}>Cancel</Text>
 </TouchableOpacity>
 </View>
 </View>)}
 </View>
 ...
 <Text style={styles.glassCardTitleText}>Health events</Text>
 </View>
 </View>
 <View style={styles.timeline}>
 {healthevents.length? (healthevents.map((ev, idx) => (<View key={`${ev.date}-${idx}`} style={styles.timelineItem}>
 <View style={styles.timelineDot} />
 <Text style={styles.timelineDate}>{ev.date}</Text>
 <Text style={styles.timelineText}>{ev.text}</Text>
 </View>))): (<View style={styles.timelineItem}>
 <Text style={[styles.timelineText, { color: colors.muted }]}>NoneHealth events</Text>
 </View>)}
 </View>
 </View>
 </View>
...
const styles = StyleSheet.create({
 ...
 borderWidth: 1,
 borderColor: 'rgba(255,255,255,0.6)',...shadows.sm,
 },
 profileCard: {
 ...
 borderRadius: 36,
 borderWidth: 3,
 borderColor: colors.card,...shadows.md,
 overflow: 'hidden',
