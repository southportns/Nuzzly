...
const CAT_BREEDS = ['British Shorthair', 'Ragdoll', 'Munchkin', 'Persian', 'Siamese', 'American Shorthair', 'Orange Tabby', 'Li Hua'];
const DOG_BREEDS = ['Corgi', 'Golden Retriever', 'Poodle', 'Husky', 'Shiba Inu', 'Labrador', 'Border Collie', 'Samoyed'];

const REPORT_CATEGORIES = [{ value: 'spam', label: 'Spam' },
 { value: 'violence', label: 'Violence' },
 { value: 'pornography', label: 'Adult Content' },
 { value: 'political', label: 'Political Sensitivity' },
...
export default function CommunityScreen() {
 ...
 const breedOptionsStatic = petType === 'cat'? ['All Breeds',...CAT_BREEDS]: petType === 'dog'? ['All Breeds',...DOG_BREEDS]: ['All Breeds',...CAT_BREEDS,...DOG_BREEDS];

 const breedOptions = breedCache.length > 0? breedCache: breedOptionsStatic;
 const filteredBreeds = breedSearch.trim()? breedOptions.filter((b) => b.toLowerCase().includes(breedSearch.trim().toLowerCase())): breedOptions;
 ...
 setBreedCache(['All Breeds',...unique]);
 } catch {
 ...
 try {
 const { data } = await supabase.from('pets').select('id, name, species, breed, photo_url').eq('profile_id', userId).eq('is_active', true).order('created_at', { ascending: true });
 ...
 if (error) {
 show('Report failed, please try again later', 'error');
 } else {
 show('Report submitted, we will review it soon', 'success');
 ...
 if (error) show('Delete Failed', 'error');
 else show('Deleted', 'success');
 }, [deletePost, show]);

 const triggerFileIn = useCallback(async () => {
 ...
 show('Image size cannot exceed 5MB', 'warning');
 return false;
 }
 return true;
 });
 ...
 if (!agreedToTerms) {
 show('Please firstCommunityandPrivacy Policy', 'warning');
 return;
 }
 if (!phoneVerified) {
 ...
 if (error) {
 show('PostFailed', 'error');
 return;
 }

 show('PostSuccess', 'success');
 ...
 show(e.message || 'Failed to send verification code', 'error');
 }
 }, [phoneNumber, show]);
 ...
 show(e.message || 'VerifyFailed', 'error');
 }
 ...
 {profile?.avatar_url? (<Image source={{ uri: profile.avatar_url }} style={styles.postAvatar} />): (<View style={[styles.postAvatar, styles.postAvatarFallback]}>
 <Text style={styles.postAvatarFallbackText}>{(profile?.display_name || '?')[0]}</Text>
 </View>)}
 <Text style={styles.postUsername} numberOfLines={1}>{profile?.display_name || 'Anonymous'}</Text>
 </View>
 <View style={styles.postContent}>
 <Text style={styles.postText} numberOfLines={5}>{post.content}</Text>
 {post.images && post.images.length > 0 && (<View style={[styles.postImages, layout === 'single' && styles.postImagesSingle, layout === 'double' && styles.postImagesDouble]}>
 {post.images.map((img, idx) => (<TouchableOpacity
 key={idx}
 activeOpacity={0.9}
 ...
 {post.profile_id === currentUserId && (<TouchableOpacity style={styles.postAction} onPress={() => handleDelete(post)} activeOpacity={0.7}>
 ...
 const renderCreateModal = () => (<Modal visible={showCreatePost} animationType="slide" transparent onRequestClose={() => setShowCreatePost(false)}>
 ...
 {!phoneVerified && (<TouchableOpacity style={styles.verifyBanner} onPress={() => setShowPhoneVerify(true)} activeOpacity={0.8}>
 <Ionicons name="shield-checkmark-outline" size={18} color={colors.warning} />
 <Text style={styles.verifyBannerText}>Phone verification required before posting. Tap to verify.</Text>
 </TouchableOpacity>)}
 <TextIn
 value={newContent}
 onChangeText={setNewContent}
 placeholder="Share your story with your fur baby..."
 ...
 {previewUrls.length > 0? (<View style={styles.imagePreviewGrid}>
 {previewUrls.map((url, idx) => (<View key={idx} style={styles.previewItem}>
 ...
 {previewUrls.length < 9 && (<TouchableOpacity style={styles.previewAdd} onPress={triggerFileIn} activeOpacity={0.8}>
 <Ionicons name="add" size={24} color={colors.muted} />
 </TouchableOpacity>)}
 </View>): (<TouchableOpacity style={styles.addImageRow} onPress={triggerFileIn} activeOpacity={0.8}>
 <Ionicons name="image-outline" size={20} color={colors.muted} />
 <Text style={styles.addImageText}>Add Image</Text>
 </TouchableOpacity>)}

 {userPets.length > 0 && (<View style={styles.petSelectSection}>
 <Text style={styles.petSelectLabel}>Link Pet</Text>
 ...
 activeOpacity={0.8}
 >
 {pet.photo_url? (<Image source={{ uri: pet.photo_url }} style={styles.petSelectAvatar} />): (<View style={[styles.petSelectAvatar, styles.petSelectAvatarFallback]}>
 ...
 {pet.breed || (pet.species === 'cat'? 'Cat': pet.species === 'dog'? 'Dog': 'Other')}
 </Text>
 </View>
 </TouchableOpacity>);
 })}
 ...
 <Text style={styles.link} onPress={() => setShowAgreement(true)}>Community Guidelines</Text>
 and
 ...
 const renderReportModal = () => (<Modal visible={showReportModal} animationType="fade" transparent onRequestClose={() => setShowReportModal(false)}>
 ...
 {REPORT_CATEGORIES.map((cat) => (<TouchableOpacity
 key={cat.value}
 ...
 const renderPhoneVerifyModal = () => (<Modal visible={showPhoneVerify} animationType="fade" transparent onRequestClose={() => setShowPhoneVerify(false)}>
 <View style={styles.modalOverlay}>
 ...
 <Text style={styles.modalTitle}>Phone Verify</Text>
 <TouchableOpacity
 ...
 <View style={styles.modalBody}>
 <Text style={styles.verifyHint}>Phone verification is required before posting.</Text>
 <TextIn
 value={phoneNumber}
 ...
 maxLength={6}
 style={[styles.verifyIn, styles.verifyCodeIn]}
 />
 <TouchableOpacity
 ...
 activeOpacity={0.8}
 >
 <Text style={styles.verifyCodeBtnText}>{countdown > 0? `${countdown}s`: 'Get Code'}</Text>
 </TouchableOpacity>
 </View>
 </View>
 </View>
 ...
 const renderAgreementModal = (title: string, content: React.ReactNode) => (<Modal visible={title === 'Community'? showAgreement: showPrivacy} animationType="slide" transparent onRequestClose={() => (title === 'Community'? setShowAgreement(false): setShowPrivacy(false))}>
 <View style={styles.modalOverlay}>
 <Pressable style={styles.modalBackdrop} onPress={() => (title === 'Community'? setShowAgreement(false): setShowPrivacy(false))} />
 <View style={[styles.modalContent, { paddingBottom: insets.bottom + spacing.md }]}>
 <View style={styles.modalHeader}>
 <TouchableOpacity onPress={() => (title === 'Community'? setShowAgreement(false): setShowPrivacy(false))} style={styles.modalClose} activeOpacity={0.7}>
 <Ionicons name="close" size={18} color={colors.muted} />
 </TouchableOpacity>
 ...
 {['', 'cat', 'dog'].map((t) => (<TouchableOpacity
 key={t || 'all'}
 ...
 <Pressable
 style={[styles.searchBreedWrapper, (showBreed || selectedBreed!== 'All Breeds') && styles.searchBreedWrapperActive]}
 onPress={() => setShowBreed(true)}
 >
 <Ionicons name="search" size={16} color={(showBreed || selectedBreed!== 'All Breeds')? colors.primary: colors.muted} />
 <TextIn
 value={breedSearch}
 onChangeText={(text) => { setBreedSearch(text); setShowBreed(true); }}
 placeholder={selectedBreed === 'All Breeds'? 'Search breed...': selectedBreed}
 placeholderTextColor={(showBreed || selectedBreed!== 'All Breeds')? colors.primary: colors.muted}
 style={styles.breedSearchIn}
 onFocus={() => setShowBreed(true)}
 />
 {selectedBreed!== 'All Breeds' && (<TouchableOpacity
 style={styles.clearBtn}
 onPress={() => { setSelectedBreed('All Breeds'); setBreedSearch(''); refresh(); }}
 activeOpacity={0.7}
 >
 ...
 {showBreed && (<Pressable style={styles.breedDropdownBackdrop} onPress={() => setShowBreed(false)}>
 <View style={styles.breedDropdown}>
 {filteredBreeds.length === 0? (<Text style={styles.dropdownEmpty}>No matching breeds found</Text>): (filteredBreeds.map((b) => (<TouchableOpacity
 key={b}
 ...
 >
 {loading && posts.length === 0? (<View style={styles.emptyState}>
 <Text style={styles.emptyIcon}>🐾</Text>
 <Text style={styles.emptyText}>Loading...</Text>
 </View>):!loading && posts.length === 0? (<View style={styles.emptyState}>
 <Text style={styles.emptyIcon}>📝</Text>
 <Text style={styles.emptyText}>No posts yet. Be the first to post!</Text>
 </View>): (<>
 {posts.map(renderPost)}
 {loading && posts.length > 0 && (<View style={styles.loadingMore}>
 <ActivityIndicator size="small" color={colors.primary} />
 <Text style={styles.loadingMoreText}>Load More...</Text>
 </View>)}
 {!hasMore && posts.length > 0 && <Text style={styles.noMore}>No more posts</Text>}
 </>)}
 ...
 {renderAgreementModal('Community', (<>
 <Text style={styles.agreementH3}>1. Community Guidelines</Text>
 <Text style={styles.agreementP}>1., Good, Not Post,, Content. </Text>
 <Text style={styles.agreementP}>2. Not PostFalse, believe, includingbut Not on FalseProductRecommendations, Not. </Text>
 <Text style={styles.agreementP}>3. Not Post,, etc.Content. </Text>
 <Text style={styles.agreementP}>4. Not Postand,, Content. </Text>
 <Text style={styles.agreementP}>5. Not Postand profilePrivacy believe(if Phone Number, etc.). </Text>
 <Text style={styles.agreementH3}>2. Violation Handling</Text>
 <Text style={styles.agreementP}>Content violating community guidelines will be deleted. Severe violations may result in posting restrictions.</Text>
 <Text style={styles.agreementH3}>3. Minor Protection</Text>
 <Text style={styles.agreementP}>Users under 14 require guardian consent. Posting is restricted from 10 PM to 6 AM.</Text>
 </>))}
 {renderAgreementModal('Privacy Policy', (<>
 <Text style={styles.agreementH3}>Information Collection</Text>
 <Text style={styles.agreementP}>your Phone Numberuse on Authentication, yourPost andImageContentuse on Community. </Text>
 <Text style={styles.agreementH3}>Information Usage</Text>
 <Text style={styles.agreementP}>Phone Numberuse on Authentication, Not OtherUser. Post Content Root"Security" 6months. </Text>
 <Text style={styles.agreementH3}>Information Protection</Text>
 <Text style={styles.agreementP}>We use industry-standard encryption to protect your information and never sell your data to third parties.</Text>
 <Text style={styles.agreementH3}>Your Rights</Text>
 <Text style={styles.agreementP}>You have the right to view, modify, and delete your information. Account deletion will anonymize your posts (shown as "Deleted User").</Text>
 </>))}
...
const styles = StyleSheet.create({
 ...
 borderColor: colors.primary,...shadows.btn,
 },
 breedSearchIn: {
 flex: 1,
 fontSize: typography.sizes.base,
