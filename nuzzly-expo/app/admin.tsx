...
export default function AdminScreen() {
  ...
  const statCards = useMemo(
    () => [
      { label: '总用户', value: stats.userCount, accent: STAT_ACCENTS[0] },
      { label: '总宠物', value: stats.petCount, accent: STAT_ACCENTS[1] },
      { label: '产品数', value: stats.productCount, accent: STAT_ACCENTS[2] },
      { label: '评价数', value: stats.reviewCount, accent: STAT_ACCENTS[3] },
      { label: '近7天新评价', value: stats.reviewLast7d, accent: STAT_ACCENTS[4] },
      { label: '被标记用户', value: stats.flaggedCount, accent: STAT_ACCENTS[5] },
    ],
    [stats]
  );
  ...
  async function loadStats() {
    ...
    const [users, pets, products, reviews, recent, flagged, recentCount] = await Promise.all([
      ...
      supabase
        ...
        .select(
          'id, overall_rating, review_text, created_at, public_profiles!inner(username, display_name), products!inner(name)'
        )
        .order('created_at', { ascending: false })
        .limit(5),
      supabase
        .from('public_profiles')
        .select('id, username, display_name, trust_score, flag_reason')
        .eq('is_flagged', true)
        .limit(5),
  ...
  return (
    <View style={[styles.shell, { paddingTop: insets.top }]}>
      <PageHeader title="管理员控制台" actionText="Admin" actionDisabled />
      <ScrollView
        ...
        {loading ? (
          ...
          <>
            <View style={styles.statsGrid}>
              {statCards.map((s) => (
                <View key={s.label} style={[styles.statCard, { borderLeftColor: s.accent }]}>
            ...
            <View style={styles.section}>
              <View style={styles.sectionHeader}>
                <Text style={styles.sectionTitle}>最近评价</Text>
                <Text style={styles.sectionCount}>共 {recentReviews.length} 条</Text>
              </View>
              {recentReviews.length ? (
                ...
                <View style={styles.emptyMini}>
                  <Text style={styles.emptyMiniText}>暂无最近评价</Text>
                </View>
              )}
            </View>
            ...
            <View style={styles.section}>
              <View style={styles.sectionHeader}>
                <Text style={styles.sectionTitle}>被标记用户</Text>
              </View>
              {flaggedUsers.length ? (
                <View style={styles.listCard}>
                  {flaggedUsers.map((u) => (
                    <View key={u.id} style={styles.userItem}>
                      ...
                      <Text style={styles.userScore}>信任分 {u.trust_score ?? 0}</Text>
                    </View>
                  ))}
                </View>
              ) : (
                <View style={styles.emptyMini}>
                  <Text style={styles.emptyMiniText}>
                    <Text style={{ color: colors.success }}>社区秩序良好</Text>，暂无被标记用户
                  </Text>
                </View>
              )}
            </View>
