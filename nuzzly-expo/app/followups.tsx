...
export default function FollowingupsScreen() {
 const router = useRouter();
 const insets = useSafeAreaInsets();
 const { schedules, loading, fetchSchedules } = useFollowingups();
 ...
 const [pending, completed, overdue] = await Promise.all([fetchSchedules('pending', { silent: true }),
 ...
 function onTap(s: FollowingupSchedule) {
 ...
 const emptyTitle =
 tab === 'pending'? 'NonePending Tracking': tab === 'completed'? 'NoneCompleted Tracking': 'NoneExpiredTracking';
 const emptyDesc =
 tab === 'pending'? 'SubmitProductReviewafter, 7/14/30/60/90/180 daysTrackingPlan': '';
 const emptyIcon = tab === 'pending'? '📝': tab === 'completed'? '✅': '⏰';

 return (<View style={[styles.shell, { paddingTop: insets.top }]}>
 <PageHeader title="Long-term Tracking" />
 ...
 {count > 0? (<View style={styles.segBadge}>
 ...
 >
 {loading? (<View style={styles.loadingList}>
 {[1, 2, 3].map((i) => (<View key={i} style={styles.skCard}>
 ...
 </View>): schedules.length? (<View style={styles.scheduleList}>
 {schedules.map((s) => {
 ...
 {product?.image_url? (<Image source={{ uri: product.image_url }} style={styles.cardImg} />): (<Text style={styles.imgPh}>
 ...
 {product?.name || 'Not Product'}
 </Text>
 <Text style={styles.cardMeta}>
 {pet?.name || 'Pet'} · {pet?.breed || 'Unknown'}
 </Text>
 <View style={styles.cardFoot}>
 <View style={styles.dayTag}>
 <Text style={styles.dayTagText}>Day {s.followup_day}</Text>
 </View>
 <Text style={styles.dueTag}>to term: {formatDate(s.due_date)}</Text>
 ...
 {showArrow? (<Ionicons name="chevron-forward" size={18} color={colors.muted} />): null}
 </TouchableOpacity>);
 })}
 </View>): (<EmptyState
 icon={emptyIcon}
 ...
 actionText={tab === 'pending'? 'ProductsReview': undefined}
