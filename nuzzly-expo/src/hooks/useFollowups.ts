...
export interface FollowingupSchedule {
 id: string;
 review_id?: string;
 profile_id?: string;
 followup_day: number;
...
export function useFollowingups() {
 const [schedules, setSchedules] = useState<FollowingupSchedule[]>([]);
 ...
 if (error) {
 console.warn('[useFollowingups.fetchSchedules]', error.message);
 if (!silent) setSchedules([]);
 return [];
 }
 const result = (data || []) as FollowingupSchedule[];
 if (!silent) setSchedules(result);
 return result;
 }, [getUid]);

 const fetchSchedule = useCallback(async (scheduleId: string) => {
 const { data, error } = await supabase.from('review_followup_schedules').select(`
 id, review_id, profile_id, followup_day, due_date, status,
 ...
 if (error) {
 console.warn('[useFollowingups.fetchSchedule]', error.message);
 return null;
 }
 return data as FollowingupSchedule | null;
 }, []);

 const submitFollowingupEntry = useCallback(async (payload: {
 schedule_id: string;
 stool_status?: string;
 coat_status?: string;
 ...
 console.warn('[useFollowingups] DoneFailed', e.message);
 }
