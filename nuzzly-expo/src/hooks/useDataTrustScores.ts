...
export function useDataTrustScores() {
 ...
 setLoading(true);
 let query = supabase.from('data_trust_scores').select('*').order('calculated_at', { ascending: false });
 ...
 const { data, error } = await supabase.from('data_trust_scores').select('*').eq('entity_type', entityType).eq('entity_id', entityId).order('calculated_at', { ascending: false }).limit(1).single();
 ...
 const { data, error } = await supabase.from('data_trust_scores').upsert({
 ...
 if (score == null) return { label: 'Unknown', color: '#8b949e', icon: '?' };
 if (score >= 0.9) return { label: 'Very premiumbelieve', color: '#3fb950', icon: '🛡️' };
 if (score >= 0.7) return { label: 'premiumbelieve', color: '#58a6ff', icon: '✓' };
 if (score >= 0.5) return { label: 'Mediumetc.believe', color: '#d29922', icon: '⚠' };
 if (score >= 0.3) return { label: 'Lowbelieve', color: '#f85149', icon: '!' };
 return { label: 'extremelyLowbelieve', color: '#da3633', icon: '🚨' };
 }, []);

 const getSuspiciousLevelLabel = useCallback((level?: number) => {
 if (level == null) return 'Unknown';
 if (level >= 0.8) return 'premium ';
 ...
 has_photos: ' Photo',
 has_voucher: ' Purchased',
 has_long_term_data: ' Long-termData',
 is_continuous: 'Data',
 is_anomaly: 'Data',
 };
 return labels[factor || ''] || factor || 'Unknowndue to ';
 }, []);
