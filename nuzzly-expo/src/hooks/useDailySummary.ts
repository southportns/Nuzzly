...
const RISK_LABELS: Record<string, string> = { low: 'LowRisk', medium: 'MediumRisk', high: 'premiumRisk', critical: 'Severity' };
...
const ANOMALY_LABELS: Record<string, string> = {
 appetite_anomaly: 'Appetite',
 weight_anomaly: 'Weight',
 activity_anomaly: '',
 stool_anomaly: '',
 symptom_anomaly: 'Symptom',
 medication_anomaly: 'Medication',
};
...
export function useDailySummary() {
 ...
 const { data, error } = await supabase.from('daily_summary').select('*').eq('pet_id', petId).gte('date', startDateStr).order('date', { ascending: false });
 ...
 if (existing?.id) {
 const { data, error } = await supabase.from('daily_summary').update({
 ...
 const { data, error } = await supabase.from('daily_summary').insert({
 pet_id: petId,
 date: targetDate,
 risk_level: summaryData.risk_level,
 summary_text: summaryData.summary_text,
 ...
 const getRiskLevelLabel = useCallback((level?: string) => RISK_LABELS[level || ''] || 'Unknown', []);
 ...
 const riskScores = summaries.map((s) => ({ low: 1, medium: 2, high: 3, critical: 4 }[s.risk_level || 'low'] || 1)).reverse();
