...
export function useContentModeration() {
  ...
  const rejectPost = useCallback(async (postId: string, reason?: string) => {
    ...
    const { error } = await supabase
      .from('community_posts')
      .update({ review_status: 'rejected', reject_reason: reason || '内容不符合社区规范' })
      .eq('id', postId);
    if (error) throw new Error(error.message);
    setModerationQueue((prev) => prev.filter((p) => p.id !== postId));
  ...
  const getReviewStatusLabel = useCallback((status?: string) => STATUS_LABELS[status || ''] || status || '未知', []);
