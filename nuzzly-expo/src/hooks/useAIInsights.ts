...
export function useAIInsights() {
 ...
 setLoading(true);
 try {
 let query = supabase.from('ai_insights').select('*').eq('is_published', true).order('created_at', { ascending: false }).limit(limit);
...
export function getInsightTypeLabel(type: string) {
 ...
 price_alert: 'PriceReminder',
 risk_warning: 'Risk',
 recommendation: 'RecommendationsAdvice',
 trend_analysis: 'Analysis',
 ingredient_change: 'Ingredientsmore',
 health_tip: 'HealthNotice',
 recall_notice: 'Notifications',
 };
